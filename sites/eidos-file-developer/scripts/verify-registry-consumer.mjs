...
for (const [name, version] of Object.entries(expected)) {
  ...
  const validRequest = allowTarballs
    ...
    : requested === version
  if (!validRequest) throw new Error(`${name} must stay pinned to ${version}`)
  ...
  if (installed === resolve(root, "..", "..", "packages", name.split("/")[1])) {
    throw new Error(`${name} resolved to the Eidos monorepo`)
  }
