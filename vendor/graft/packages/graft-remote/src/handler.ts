...
export function createGraftRemoteHandler<
  ...
  return async (request): Promise<Response> => {
    try {
      ...
      if (!(error instanceof GraftProtocolError)) {
        console.error(
          JSON.stringify({ message: "unhandled graft remote error" })
        )
      }
...
function parseOperation(value: string | undefined): GraftRemoteOperation {
  ...
  if (!OPERATIONS.has(value as GraftRemoteOperation)) {
    throw new GraftProtocolError(
      404,
      "operation_not_found",
      "Unknown remote protocol operation"
    )
  }
  ...
  if (operation === "descriptor" || operation === "list") {
    if (value !== undefined) {
      throw new GraftProtocolError(
        400,
        "invalid_list_path",
        "The operation has no path suffix"
      )
    }
    return undefined
  }
...
async function compareAndSwap(
  ...
  if (!(await backend.compareAndSwap(path, expected, replacement))) {
    throw new GraftProtocolError(
      409,
      "compare_failed",
      "Object changed during compare-and-swap"
    )
  }
  return emptyResponse()
}
...
async function compareAndDelete(
  ...
  if (!(await backend.compareAndDelete(path, expected))) {
    throw new GraftProtocolError(
      409,
      "compare_failed",
      "Object changed during compare-and-delete"
    )
  }
  return emptyResponse()
}
...
async function listObjects(
  ...
  if (result.paths.length > query.limit) {
    throw backendContractError(
      "List backend returned more paths than requested"
    )
  }
  ...
  for (const path of result.paths) {
    ...
    if (
      ...
      throw backendContractError(
        "List backend returned unsorted or out-of-prefix paths"
      )
    }
    previous = path
  }
...
function asBodyInit(object: GraftObject): BodyInit {
  if (object.body instanceof Uint8Array) {
    if (
      object.body.byteOffset === 0 &&
      object.body.byteLength === object.body.buffer.byteLength
    ) {
