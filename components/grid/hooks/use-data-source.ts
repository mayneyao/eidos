...
import {
  columnsHandleMap,
  getRawCellValueOrAutoGeneratedColumnCellValue,
} from "../helper"
    ...
    (rowData: any, col: number) => {
      ...
      if (!colHandle) {
        ...
        if (FieldClass) {
          ...
          const _cv = getRawCellValueOrAutoGeneratedColumnCellValue(
            rowData,
            cv,
            field.type
          )
          return fieldInstance.getCellContent(_cv as never, { userMap })
        } else {
