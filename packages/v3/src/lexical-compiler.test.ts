  ...
  describe("compileLexicalCode", () => {
    it("should transform named imports from lexical packages", async () => {
      ...
      expect(result.code).toContain(
        'const { $createParagraphNode, $getRoot } = window["__LEXICAL"]'
      )
      expect(result.code).toContain(
        'const { $isAtNodeEnd } = window["__@LEXICAL/UTILS"]'
      )
    })
    ...
    it("should transform default imports from lexical packages", async () => {
      ...
      expect(result.error).toBeNull()
      expect(result.code).toContain(
        'const MarkdownPlugin = window["__@LEXICAL/MARKDOWN"]'
      )
      expect(result.code).toContain(
    ...
    it("should handle mixed import types", async () => {
      ...
      expect(result.code).toContain(
        'const { namedExport } = window["__LEXICAL"]'
      )
      expect(result.code).toContain(
        'const { util1, util2 } = window["__@LEXICAL/UTILS"]'
      )
    })
    ...
    it("should handle multiple imports from the same package", async () => {
      ...
      expect(result.error).toBeNull()
      expect(result.code).toContain(
        'const { $createParagraphNode } = window["__LEXICAL"]'
      )
