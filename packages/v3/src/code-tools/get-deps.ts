...
export async function generateImportMap(
  ...
  localLibs.forEach((dep) => {
    ...
    imports[dep] =
      `http://sandbox.${spaceId}.eidos.localhost:13127/${targetPath}.js?no-rewrite=1`
  })
  ...
  thirdPartyLibs.forEach((dep) => {
    ...
    if (!imports[dep]) {
      ...
      imports[dep] =
        `https://${ESM_SERVER}/${dep}${versionSuffix}?external=react,react-dom`
    }
  })
  ...
  const linkCreationStatements = cssLibs
    .map((cssFile) => {
      let cssUrl = ""
...
export async function getAllLibs(
  ...
  for (const dep of dependencies) {
    if (dep.endsWith(".css")) {
      ...
      // Handle hooks as potential UI libs
  ...
  while (queue.length > 0) {
    ...
    const componentDeps =
      uiComponentsDependencies[
        uiLibName as keyof typeof uiComponentsDependencies
      ]
    if (componentDeps) {
      if (componentDeps.thirdPartyLibs) {
        componentDeps.thirdPartyLibs.forEach((lib) =>
          thirdPartyLibsMasterSet.add(lib)
        )
      }
  ...
  if (getLocalLibCode) {
    ...
    while (localLibsQueue.length > 0) {
      ...
      if (localLibCode) {
        ...
        localLibDeps.thirdPartyLibs.forEach((lib) =>
          thirdPartyLibsMasterSet.add(lib)
        )
        ...
        localLibDeps.localLibs.forEach((newLocalLib) => {
          if (
            !localLibsSet.has(newLocalLib) &&
            !processedLocalLibs.has(newLocalLib)
          ) {
