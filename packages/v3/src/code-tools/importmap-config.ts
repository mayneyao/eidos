...
export function buildEsmUrl(
  ...
  const externalParam =
    external && external.length > 0 ? `?external=${external.join(",")}` : ""
...
export function getCoreImportMap(): Record<string, string> {
  const imports: Record<string, string> = {
    ...
    "tailwind-merge": buildEsmUrl(
      "tailwind-merge",
      CORE_DEPENDENCIES["tailwind-merge"]
    ),
    "class-variance-authority": buildEsmUrl(
      "class-variance-authority",
      CORE_DEPENDENCIES["class-variance-authority"]
    ),
    ...
    // Eidos packages
    "@eidos.space/react": buildEsmUrl(
      "@eidos.space/react",
      EIDOS_PACKAGES["@eidos.space/react"],
      ["react", "zustand"]
    ),

    // Radix UI packages
    "@radix-ui/react-icons": buildEsmUrl(
      "@radix-ui/react-icons",
      RADIX_PACKAGES["@radix-ui/react-icons"],
      ["react", "react-dom"]
    ),
    "@radix-ui/react-toast": buildEsmUrl(
      "@radix-ui/react-toast",
      RADIX_PACKAGES["@radix-ui/react-toast"],
      ["react", "react-dom"]
    ),
  }
