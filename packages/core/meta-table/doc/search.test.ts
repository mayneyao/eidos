...
describe("performTrigramSearch", () => {
  ...
  describe("truncation", () => {
    it("should truncate long text before applying highlights", async () => {
      const longText =
        "This is a very long document that contains some test content and more text"
      const mockDataSpace = new MockDataSpace([{ id: 1, markdown: longText }])
    ...
    it("should handle truncation with multiple keywords", async () => {
      const longText =
        "This document contains testing and best practices for development"
      const mockDataSpace = new MockDataSpace([{ id: 1, markdown: longText }])
