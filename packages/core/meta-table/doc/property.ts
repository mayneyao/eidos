...
export function WithProperty<T extends Constructor>(Base: T) {
  return class PropertyDocTableMixin extends Base {
     ...
     */
    async ensureCustomPropertyColumns(
    ...
    async getCustomProperties(id: string) {
      try {
        ...
        const customPropertyColumns = columns.filter(
          (col: string) => !RESERVED_PROPERTIES.includes(col)
        )
    ...
    async getProperties(id: string) {
      ...
      const columns = await this.dataSpace.column.list({
        table_name: this.name,
      })
      ...
      const selectClauses = buildSelectClauseWithCasting(
        allColumnNames,
        propertyTypes
      )
    ...
    async setProperties(id: string, properties: Record<string, any>) {
      ...
      try {
        ...
        const setClauses = Object.keys(validProperties)
          .map((prop) => `${prop} = ?`)
          .join(", ")
        ...
        return {
          success: true,
          updatedProperties: Object.keys(validProperties),
        }
      } catch (error) {
    ...
    async addDisplayProperty(
      ...
      try {
        ...
        return {
          success: false,
          message: `Failed to add display property: ${error}`,
        }
      }
    }
    ...
    async removeDisplayProperty(
      ...
      try {
        ...
        return {
          success: false,
          message: `Failed to remove display property: ${error}`,
        }
      }
    }
    ...
    async setDisplayProperties(
      ...
      try {
        ...
        const columns = await this.getTableColumns()
        const invalidProperties = propertyNames.filter(
          (prop) => !columns.includes(prop)
        )
        ...
        return {
          success: false,
          message: `Failed to set display properties: ${error}`,
        }
      }
    }
    ...
    async deleteProperty(propertyName: string) {
      try {
        ...
        this.dataSpace.db
          ...
          .run([propertyName, this.name])
        this.dataSpace.db
          .prepare(`ALTER TABLE ${this.name} DROP COLUMN ${propertyName};`)
          .run()
