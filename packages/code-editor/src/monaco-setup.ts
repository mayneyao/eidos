...
export function setupMonacoEnvironment(): Promise<typeof monaco> {
  ...
  return loader.init().then(async (monacoInstance) => {
    ...
    console.log(
      "📋 Available languages:",
      supportedLanguages.map((lang) => lang.id).join(", ")
    )
    ...
    console.log(
      "✅ Monaco setup complete - plugin initialization deferred to editor instances"
    )
...
export function createModelSafely(
  ...
  if (existingModel) {
    ...
    if (existingModel.getLanguageId() !== language) {
      console.log(
        `Updating language from ${existingModel.getLanguageId()} to ${language}`
      )
  ...
  try {
    ...
    console.log(
      `Created new model: ${uri.toString()} with language: ${model.getLanguageId()}`
    )
