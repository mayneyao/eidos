...
describe("URLResolver", () => {
  ...
  describe("resolvePackageUrl", () => {
    ...
    it("should resolve scoped packages correctly", () => {
      expect(resolver.resolvePackageUrl("@babel/core")).toBe(
        "https://esm.sh/@babel/core"
      )
      expect(resolver.resolvePackageUrl("@excalidraw/excalidraw")).toBe(
        "https://esm.sh/@excalidraw/excalidraw"
      )
    })

    it("should handle version specifications", () => {
      expect(resolver.resolvePackageUrl("react@18.0.0")).toBe(
        "https://esm.sh/react@18.0.0"
      )
      expect(resolver.resolvePackageUrl("@babel/core@7.20.0")).toBe(
        "https://esm.sh/@babel/core@7.20.0"
      )
    })

    it("should handle subpath imports", () => {
      expect(resolver.resolvePackageUrl("lodash/debounce")).toBe(
        "https://esm.sh/lodash/debounce"
      )
      expect(resolver.resolvePackageUrl("@babel/core/lib/transform")).toBe(
        "https://esm.sh/@babel/core/lib/transform"
      )
    })
    ...
    it("should handle Node.js builtin modules", () => {
      ...
      expect(resolver.resolvePackageUrl("node:crypto")).toBe(
        "https://esm.sh/crypto"
      )
    })
    ...
    it("should leave relative imports unchanged", () => {
      ...
      expect(resolver.resolvePackageUrl("../../components")).toBe(
        "../../components"
      )
    })
    ...
    it("should handle resolve options", () => {
      // Target environment
      expect(resolver.resolvePackageUrl("react", { target: "deno" })).toBe(
        "https://esm.sh/react?target=deno"
      )

      // Version override
      expect(resolver.resolvePackageUrl("react", { version: "17.0.0" })).toBe(
        "https://esm.sh/react@17.0.0"
      )

      // No types
      expect(resolver.resolvePackageUrl("react", { noDts: true })).toBe(
        "https://esm.sh/react?no-dts=true"
      )

      // Development mode
      expect(resolver.resolvePackageUrl("react", { dev: true })).toBe(
        "https://esm.sh/react?dev=true"
      )

      // Bundle format
      expect(resolver.resolvePackageUrl("react", { format: "cjs" })).toBe(
        "https://esm.sh/react?bundle=cjs"
      )

      // Multiple options
      expect(
        resolver.resolvePackageUrl("react", {
          target: "deno",
          version: "18.0.0",
          noDts: true,
        })
  ...
  describe("getPackageMetadata", () => {
    ...
    it("should parse packages with subpaths", () => {
      ...
      const scopedMetadata = resolver.getPackageMetadata(
  ...
  describe("configuration", () => {
    it("should allow setting custom ESM server URL", () => {
      ...
      expect(resolver.resolvePackageUrl("react")).toBe(
