...
import type { User, UserWithRole } from "./b"
import { UserService, UserRole, createDefaultUser } from "./b"
...
class UserManager {
  ...
  async batchUpdateUsers(
    updates: Array<{ id: number; data: Partial<Omit<User, "id">> }>
  ): Promise<User[]> {
    const results: User[] = []
  ...
  createActivityLog(
    userId: number,
    action: string
  ): { id: string; userId: number; action: string; timestamp: string } {
    return {
      id: uuidv4(),
      userId,
      action,
