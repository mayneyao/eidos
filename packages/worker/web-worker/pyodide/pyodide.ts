...
async function loadPyodideAndPackages(
  ...
  if (config?.preloadPackages?.length) {
    ...
    await Promise.all(
      config.preloadPackages.map((pkg) => micropip.install(pkg))
    )
    ...
    switch (type) {
      case "PythonScriptInstall": {
        ...
        const installResults = await Promise.all(
          payload.dependencies.map(async (pkg) => {
            try {
              try {
                pyodide.pyimport(pkg)
      ...
      case "PythonScriptCall": {
        ...
        try {
          ...
          throw new Error(
            `Command '${payload.command}' not found or not callable`
          )
        }
        ...
        try {
          ...
          console.warn(
            "Result cannot be serialized, converting to string:",
            error
          )
