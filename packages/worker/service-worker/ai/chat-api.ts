...
export async function handleChatApi(
  ...
  if (dataspace) {
    ...
    const messageIdsToDelete = previousMessages
      ...
      const result = streamText({
        ...
        onFinish: async ({ response }) => {
          try {
            ...
            if (!assistantMessage) {
              const combinedMessage = combineAssistantMessage(
                message,
                response.messages[0]
              )
              await saveMessages(
