...
export async function getMessagesByChatId(id: string, dataspace: DataSpace) {
  const messages = await dataspace.message.list(
    { chat_id: id },
    {
      orderBy: "created_at",
      order: "ASC",
...
async function createOrUpdateMessage(
  message: ChatMessage,
  dataspace: DataSpace
) {
 ...
 */
export async function isReloadScenario(
  ...
  const hasLastAssistantMessage = messages.some(
    (msg) => msg.role === "assistant" && msg.content === lastDbMessage.content
  )
