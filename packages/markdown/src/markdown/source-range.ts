...
export type EfmSourceRangeUnavailableReason =
...
export interface EfmEditableSourceRange {
  end: number
...
export type EfmEditableSourceRangeResult =
  | { range: EfmEditableSourceRange; reason?: never }
  | { range?: never; reason: EfmSourceRangeUnavailableReason }
...
export function resolveEfmEditableSourceRange({
  ...
  const defersFootnotes = syntaxFeatures.has(MARKDOWN_FEATURES.footnote)
  const documentSegments = analysis.segments
  const editorSegments = defersFootnotes
    ? [
        ...documentSegments.filter(
          (segment) => segment.placement !== "footnote-tail"
        ),
        ...documentSegments.filter(
          (segment) => segment.placement === "footnote-tail"
        ),
  ...
  if (
    defersFootnotes &&
    selectedSegments.some((segment) => segment.placement === "footnote-tail")
  ) {
    return { reason: "pinned-footnote" }
  }
  ...
  if (
    protectedSourceIndices.some(
      (index) => documentSegments[index].placement !== "footnote-tail"
    )
  ) {
    return { reason: "discontinuous-source" }
  }
