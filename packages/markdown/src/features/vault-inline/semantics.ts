import type { EfmInlineData } from "../../nodes/efm-semantic-data"
...
export interface VaultMatch extends MarkdownSyntaxRange {
  data: EfmInlineData
}

/** Shared precedence: an outer comment/link owns its entire source range. */
export function scanVaultInline(
  source: string,
  enabled: (feature: string) => boolean,
  protectedRanges: readonly MarkdownSyntaxRange[]
): VaultMatch[] {
  const accepted: VaultMatch[] = []
  for (const match of vaultInlineMatches(source, enabled, protectedRanges).sort(
...
export function vaultInlineMatches(
  source: string,
  enabled: (feature: string) => boolean,
  protectedRanges: readonly MarkdownSyntaxRange[]
): VaultMatch[] {
