...
Current editor behavior and integration guidance for `@eidos.space/markdown`.
...
[API.md](../API.md). The separately named Obsidian document profile and its
current compatibility matrix are documented in
[OBSIDIAN-COMPATIBILITY.md](../OBSIDIAN-COMPATIBILITY.md).
...
Implementation ownership is documented in [Architecture](../architecture/README.md).

`MarkdownEditor` defaults to `profile="eidos"`, which uses
`eidosMarkdownProfile`. `profile="obsidian"` selects the separately named
experimental Obsidian codec and plugin set. This opt-in preview is incomplete
and may change between prereleases; it never changes the default Eidos profile.
Hosts may instead pass a stable `plugins` array to build a smaller
Eidos-compatible composition, or provide a custom
`MarkdownProfile` when both syntax and codec behavior change. `profile` and
`plugins` are mutually exclusive. Public contracts are available from
`@eidos.space/markdown/plugin-api`; built-in descriptors are available from
`@eidos.space/markdown/plugins`. Changing the profile or compiled plugin
...
The Eidos profile additionally enables a named presentation extension for
...
The Obsidian profile separately enables wikilinks, embeds, callouts, block IDs,
tags, inline footnotes, and comments without adding them to EFM 1.0.
