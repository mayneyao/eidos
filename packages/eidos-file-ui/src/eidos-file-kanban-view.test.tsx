...
vi.mock("./eidos-file-record-card", () => ({
  EidosFileRecordCard: ({
    row,
    onOpen,
    onDelete,
...
function isLargeKanbanGroupArray(value: unknown): value is unknown[] {
  ...
  return (
    ...
    first !== null &&
    "key" in first &&
    String(first.key).startsWith("eidos-file-kanban:")
  )
}

describe("EidosFileKanbanView", () => {
  ...
  it("loads the complete Kanban record only when its inspector opens", async () => {
    ...
    expect(
      container.querySelector('[data-testid="record-inspector-value"]')
        ?.textContent
    ).toBe("Write RFC:Loaded from the Eidos File")
    await act(async () => {
      Array.from(container.querySelectorAll("button"))
  ...
  it("serializes card moves so a failed save cannot duplicate a record", async () => {
    ...
    await act(async () => {
      kanbanMocks.onDragEnd?.({
        active: { id: "row_1" },
        over: { id: "eidos-file-kanban:__eidos_empty_group__" },
      })
      await Promise.resolve()
    })
    ...
    expect(
      container.querySelectorAll('[data-eidos-file-row-id="row_1"]')
    ).toHaveLength(1)
    expect(
      container.querySelector('[role="region"][aria-label="todo, 1 records"]')
        ?.textContent
  ...
  it("creates a record directly in its target group", async () => {
    ...
    const todoColumn = container.querySelector<HTMLElement>(
      '[data-board-id="eidos-file-kanban:todo"]'
    )
    await act(async () => {
  ...
  it("does not rerun the column virtualizer while typing a new record", async () => {
    ...
    const input = todoColumn?.querySelector<HTMLInputElement>("input")
    const virtualizer = vi.spyOn(
      eidosFileVirtualScroll,
  ...
  it("keeps failed inline creation recoverable without a global error", async () => {
    const onAddRow = vi
      .fn()
      .mockRejectedValueOnce(new Error("Eidos File is read-only"))
  ...
  it("keeps cards mounted while grouped counts and visible pages refresh", async () => {
    ...
    let resolveCounts:
      ...
      | undefined
    let resolveTodoPage: ((page: EidosFileRowPage) => void) | undefined
  ...
  it("keeps the server page cursor contiguous after moving a loaded row", async () => {
    ...
    await act(async () => {
      const scroller = container.querySelector<HTMLElement>(
        '[data-eidos-file-kanban-column-scroll="eidos-file-kanban:todo"]'
      )
      if (!scroller) return
    ...
    await act(async () => {
      const scroller = container.querySelector<HTMLElement>(
        '[data-eidos-file-kanban-column-scroll="eidos-file-kanban:done"]'
      )
      if (!scroller) return
      scroller.scrollTop = 100_000
      scroller.dispatchEvent(new Event("scroll"))
  ...
  it("loads the target group page and reveals a search result", async () => {
    ...
    expect(
      container
        .querySelector('[data-eidos-file-row-id="row_50"]')
  ...
  it("virtualizes a million-card column and loads the target window on scroll", async () => {
    ...
    const initialMountedCards = todoColumn?.querySelectorAll(
      "[data-eidos-file-row-id]"
    ).length
    ...
    expect(
      todoColumn
        ?.querySelector("[data-eidos-file-kanban-column-scroll]")
    ...
    expect(
      todoColumn?.querySelectorAll("[data-eidos-file-row-id]").length
    ).toBeLessThan(50)
  ...
  it("prefetches a group page before visible cards reach the loaded edge", async () => {
    ...
    expect(
      container.querySelectorAll("[data-eidos-file-kanban-placeholder]")
    ).toHaveLength(0)
    expect(
      container
        .querySelector(
  ...
  it("keeps mounted cards isolated when their group appends a page", async () => {
    ...
    const mountedRowId = scroller
      ?.querySelector<HTMLElement>("[data-eidos-file-row-id]")
      ?.getAttribute("data-eidos-file-row-id")
  ...
  it("does not sweep every Kanban group when one large column starts paging", async () => {
    ...
    const manyColumnTable: EidosFileTableSnapshot = {
      ...table,
      fields: table.fields.map((field) =>
        field.tableColumnName === "status"
    ...
    try {
      await act(async () => {
        const scroller = container.querySelector<HTMLElement>(
          '[data-eidos-file-kanban-column-scroll="eidos-file-kanban:status_0"]'
        )
        if (!scroller) return
        scroller.scrollTop = 100_000
        scroller.dispatchEvent(new Event("scroll"))
  ...
  it("does not rerender unaffected columns when an inspected field changes", async () => {
    ...
    const tableWithPriority: EidosFileTableSnapshot = {
      ...table,
  ...
  it("only mounts the horizontal window for a large set of columns", async () => {
    ...
    expect(recordCardMocks.disabledMoveOptionIds.get("row_status_0")).toBe(
      "eidos-file-kanban:status_0"
    )
    expect(
      Number(
        container
          .querySelector("[data-eidos-file-kanban-scroll]")
          ?.getAttribute("data-eidos-file-cached-group-windows")
      )
  ...
  it("only rebuilds the failed virtual item when a group page retries", async () => {
    ...
    expect(container.textContent).toContain("Retry loading records")
    const rowLookup = vi.spyOn(eidosFileRowWindow, "rowFromWindow")
