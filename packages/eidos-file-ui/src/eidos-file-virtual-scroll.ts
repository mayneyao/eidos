...
export const EIDOS_FILE_VIRTUAL_SCROLL_MAX_SIZE = 12_000_000
...
export const EIDOS_FILE_VIRTUAL_SCROLL_MAX_ITEMS = 2_048
const EIDOS_FILE_VIRTUAL_SCROLL_CHUNK_ITEMS = 512
const EIDOS_FILE_VIRTUAL_INDEX_ATTRIBUTE = "data-eidos-file-virtual-index"

export interface EidosFileVirtualWindow {
  start: number
  count: number
}
...
export function eidosFileVirtualPhysicalSize(logicalSize: number): number {
  return Math.min(
    finiteNonNegative(logicalSize),
    EIDOS_FILE_VIRTUAL_SCROLL_MAX_SIZE
  )
}

export function eidosFileVirtualWindowForOffset(
  totalCount: number,
  estimatedStride: number,
  logicalOffset: number
): EidosFileVirtualWindow {
  const safeCount = Math.max(0, Math.floor(totalCount))
  if (safeCount <= EIDOS_FILE_VIRTUAL_SCROLL_MAX_ITEMS) {
  ...
  const chunkStart =
    Math.floor(anchorIndex / EIDOS_FILE_VIRTUAL_SCROLL_CHUNK_ITEMS) *
    EIDOS_FILE_VIRTUAL_SCROLL_CHUNK_ITEMS
  const start = clamp(
    chunkStart - EIDOS_FILE_VIRTUAL_SCROLL_CHUNK_ITEMS,
    0,
    safeCount - EIDOS_FILE_VIRTUAL_SCROLL_MAX_ITEMS
  )
  return { start, count: EIDOS_FILE_VIRTUAL_SCROLL_MAX_ITEMS }
}

export function eidosFileVirtualLogicalOffset(
  ...
  const physicalMaximum = Math.max(
    0,
    eidosFileVirtualPhysicalSize(safeLogicalSize) - safeViewportSize
  )
...
export function eidosFileVirtualPhysicalOffset(
  logicalOffset: number,
  logicalSize: number,
  viewportSize: number
): number {
...
export function eidosFileVirtualItemOffset(
  logicalItemOffset: number,
  physicalScrollOffset: number,
  logicalSize: number,
  viewportSize: number
): number {
  const logicalScrollOffset = eidosFileVirtualLogicalOffset(
...
interface EidosFileBoundedVirtualizerOptions<
...
interface EidosFileBoundedVirtualizerGeometry {
  bounded: boolean
  totalCount: number
  estimatedStride: number
  estimatedItemSize: number
...
export interface EidosFileBoundedVirtualizerResult<
  TScrollElement extends HTMLElement,
  TItemElement extends Element,
> {
...
export function resetEidosFileVirtualizerMeasurements<
  TScrollElement extends HTMLElement,
  TItemElement extends Element,
...
export function useEidosFileBoundedVirtualizer<
  TScrollElement extends HTMLElement,
  TItemElement extends Element,
>({
  count,
...
}: EidosFileBoundedVirtualizerOptions<
  TScrollElement,
  TItemElement
>): EidosFileBoundedVirtualizerResult<TScrollElement, TItemElement> {
  ...
  const estimatedLogicalOffset = eidosFileVirtualLogicalOffset(
    physicalScrollOffset,
    estimatedLogicalSize,
    viewportSize
  )
  const virtualWindow = eidosFileVirtualWindowForOffset(
    count,
    estimatedStride,
    ...
    (
      ...
      observeElementOffset(instance, (physicalOffset, isScrolling) => {
        ...
        const nextViewportSize =
          ...
          initialRect.height
        const globalLogicalOffset = eidosFileVirtualLogicalOffset(
          physicalOffset,
          geometry.logicalSize,
          nextViewportSize
        )
        const nextWindow = eidosFileVirtualWindowForOffset(
          geometry.totalCount,
  ...
  const scrollToOffset = useCallback(
    (
      ...
      scrollElement.scrollTo({
        top: eidosFileVirtualPhysicalOffset(
          globalLogicalOffset,
          geometry.logicalSize,
          nextViewportSize
        ),
  ...
  const virtualizer = useVirtualizer<TScrollElement, TItemElement>({
    ...
    overscan,
    useAnimationFrameWithResizeObserver,
    indexAttribute: EIDOS_FILE_VIRTUAL_INDEX_ATTRIBUTE,
  ...
  const scrollToIndex = useCallback(
    (targetIndex: number, options: ScrollToOptions = {}) => {
      ...
      const currentLogicalOffset = eidosFileVirtualLogicalOffset(
  ...
  return {
    ...
    virtualItems,
    logicalSize,
    physicalSize: eidosFileVirtualPhysicalSize(logicalSize),
    physicalScrollOffset,
    logicalScrollOffset,
    localScrollOffset,
    viewportSize,
