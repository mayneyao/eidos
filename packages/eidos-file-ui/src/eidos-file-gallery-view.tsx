...
function galleryCardWidth(view: EidosFileViewInfo): number {
...
interface EidosFileGalleryVirtualRowProps {
  rowWindow: EidosFileRowWindow
...
function galleryVirtualRowPropsEqual(
  previous: EidosFileGalleryVirtualRowProps,
  next: EidosFileGalleryVirtualRowProps
): boolean {
  if (
...
const EidosFileGalleryVirtualRow = memo(function EidosFileGalleryVirtualRow({
  rowWindow,
  globalRowIndex,
  virtualIndex,
  offset,
...
export const EidosFileGalleryView = memo(function EidosFileGalleryView({
  table,
  view,
  disabled = false,
  reloadToken = 0,
  ...
  const previousViewPropertiesRef =
    useRef<EidosFileViewInfo["properties"]>(null)
  ...
  const [rowWindow, setRowWindow] = useState<EidosFileRowWindow>({
    rows: [] as EidosFileRow[],
    startOffset: 0,
  ...
  const cardLayout = useMemo(
    () => createEidosFileRecordCardLayout(table.fields, view),
    [table.fields, view]
  )
  ...
  return (
    <div className="eidos-file-detail-layout flex h-full min-h-0 w-full overflow-hidden">
      <div
        ...
        data-eidos-file-column-count={columnCount}
        data-eidos-file-window-size={rows.length}
        data-eidos-file-window-start={rowWindow.startOffset}
      ...
      >
        ...
        {loadingMore ? (
          <div
            data-eidos-file-gallery-progress
