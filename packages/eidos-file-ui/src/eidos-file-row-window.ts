import type { EidosFileRow, EidosFileRowPage } from "@eidos.space/eidos-file"

export type EidosFileRowWindowMergeMode = "append" | "prepend" | "replace"

export interface EidosFileRowWindow {
  rows: EidosFileRow[]
  startOffset: number
  total: number
  nextCursor?: string
...
export interface EidosFileRowWindowRequest {
  mode: EidosFileRowWindowMergeMode
  offset: number
}

function uniqueRows(rows: EidosFileRow[]): EidosFileRow[] {
...
export function requestForRowWindow(
  ...
  visibleEnd: number,
  pageSize: number
): EidosFileRowWindowRequest | null {
