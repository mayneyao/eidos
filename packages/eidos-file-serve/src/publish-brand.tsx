...
export function publishBrandingVisible(value: string | null): boolean {
  return value !== "hide"
}

export function browserPublishBrandingVisible(): boolean {
  if (typeof document === "undefined") return true
  return publishBrandingVisible(
    document
      .querySelector('meta[name="eidos-publish-branding"]')
      ?.getAttribute("content") ?? null
  )
}

export function EidosPublishBrand() {
  return (
    <a
      className="eidos-publish-brand-inline"
      href={PUBLISH_DESTINATION}
      target="_blank"
      rel="noopener noreferrer"
      referrerPolicy="no-referrer"
      aria-label="Built with Eidos — open Eidos Publish"
    >
      <svg
        viewBox="0 0 512 512"
        fill="none"
      ...
      <span>
        Built with <strong>Eidos</strong>
      </span>
    </a>
  )
}
