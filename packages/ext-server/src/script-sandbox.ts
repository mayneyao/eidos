...
export class ScriptSandboxHandler {
  ...
  async handleSandboxRequest(
    ...
    try {
      this.logger.log(
        `Handling sandbox request for space: ${spaceId}, URL: ${url.toString()}`
      )
      ...
      this.logger.error(
        `Error handling sandbox request for space ${spaceId}: ${error.message}`
      )
  ...
  private async serveScriptFile(
    ...
    try {
      ...
      const excludeLibs = externalParam
        ? externalParam.split(",").map((lib) => lib.trim())
        : []

      if (excludeLibs.length > 0) {
        this.logger.log(
          `Excluding libraries from rewrite: ${excludeLibs.join(", ")}`
        )
      }
      ...
      if (deps.length > 0) {
        if (noRewrite) {
          this.logger.log(
            `Skipped rewriting imports for ${scriptId} due to no-rewrite=1 parameter`
          )
        } else {
      ...
      this.logger.error(
        `Error serving script file ${scriptId} for space ${spaceId}: ${error.message}`
      )
