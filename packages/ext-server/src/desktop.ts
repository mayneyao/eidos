...
export interface DesktopConfigOptions {
   ...
   */
  getSpaceRegistry: () => {
    getSpace(spaceId: string): { sync?: { enabled?: boolean } } | null | undefined;
  };
...
function createDesktopExtensionProvider(
  dataSpace: any,
  spaceRegistry: { getSpace(id: string): { sync?: { enabled?: boolean } } | null | undefined },
  spaceId: string
): ExtensionProvider {
  return {
...
export function createDesktopConfig(options: DesktopConfigOptions): ExtServerConfig {
  ...
  return {
    getExtensionProvider: async (spaceId) => {
      ...
      return createDesktopExtensionProvider(dataSpace, getSpaceRegistry(), spaceId);
    },
    ...
    dependencies: {
      ...
      generateImportMap,
      uiComponentsDependencies,
      createSandboxHandler: (getScriptCode) => new ScriptSandboxHandler(getScriptCode),
    },
