...
const DEFAULT_EXTENSION_PATTERN = /^([a-zA-Z0-9-]+)\.block\.(.*?)\.eidos\.localhost$/;
...
async function renderExtension(
  ...
  const { thirdPartyLibs, uiLibs, cssLibs, localLibs } = await dependencies.getAllLibs(
    tsCode,
  ...
  let serverSideProps = {};
  const serverActionCode = dependencies.extractFunction(tsCode, "getServerSideProps");
  if (serverActionCode) {
    try {
      const result = await runServerAction(
        `(${serverActionCode})(context)`,
        { currentSpace: provider, request: { url } }
      );
  ...
  // Generate import map
  const { importMapScript, cssLoaderScript } = await dependencies.generateImportMap(
  ...
  const allThemes = [...presetThemes, ...(config.customThemes || []), ...dynamicThemes];
...
export const createExtensionMiddleware = (config: ExtServerConfig) => {
  ...
  const sandboxPattern = config.sandboxHostnamePattern || DEFAULT_SANDBOX_PATTERN;
  ...
  return async (c: Ctx, next: () => Promise<void>) => {
    ...
    if (sandboxMatch && config.dependencies.createSandboxHandler) {
      ...
      const sandboxHandler = config.dependencies.createSandboxHandler(getScriptCode);
    ...
    if (match) {
      ...
      try {
        ...
        const html = await renderExtension(config, provider, spaceId, extension, url.toString());
        ...
        htmlHeaders.append("Cross-Origin-Opener-Policy", "same-origin-allow-popups");
        htmlHeaders.append("Cross-Origin-Resource-Policy", "cross-origin");
        
        return c.html(html, { headers: htmlHeaders });
      } catch (error: any) {
        console.error(`[ExtServer] Error processing request for ${hostname}:`, error.message);
        return c.text(`Error processing extension request: ${error.message}`, 500);
      }
    }
