...
describe("ProxyHandler", () => {
  ...
  describe("URL Validation", () => {
    it("should allow valid HTTPS URLs", () => {
      ...
      expect(handler.isValidTargetUrl("https://api.example.com/data")).toBe(
        true
      )
    ...
    it("should allow URLs with search parameters", () => {
      ...
      expect(
        handler.isValidTargetUrl(
          "http://api.example.com/data?filter=active&sort=name"
        )
      ).toBe(true)
    })
    ...
    it("should block internal domains", () => {
      ...
      expect(
        handler.isValidTargetUrl("http://sandbox.space123.eidos.localhost")
      ).toBe(false)
    })
  ...
  describe("Request Handling", () => {
    ...
    it("should handle OPTIONS requests", async () => {
      const context = createMockContext(
        "http://proxy.eidos.localhost:13127/",
        "OPTIONS"
      )
    ...
    it("should return proxy status", async () => {
      const context = createMockContext(
        "http://proxy.eidos.localhost:13127/status"
      )
    ...
    it("should handle URLs with search parameters correctly", async () => {
      ...
      const context = createMockContext(
        `http://proxy.eidos.localhost:13127/?url=${encodedUrl}`
      )
    ...
    it("should handle complex encoded search parameters", async () => {
      const targetUrl =
        'https://api.example.com/search?q=hello%20world&data={"key":"value"}'
      const encodedUrl = encodeURIComponent(targetUrl)
      const context = createMockContext(
        `http://proxy.eidos.localhost:13127/?url=${encodedUrl}`
      )
  ...
  describe("Binary Data Handling", () => {
    it("should properly handle binary file extensions", async () => {
      ...
      for (const url of binaryUrls) {
        ...
        const isBinaryContent = urlPath.match(
          /\.(bin|dat|weights|model|pb|onnx|tflite)$/
        )
