...
export async function startServer(config: HeadlessConfig): Promise<void> {
  ...
  app.use("*", async (c, next) => {
    if (process.env.DEBUG_DOMAINS === "1") {
      ...
      const isExtensionMatch = !!(
        hostnamePattern && hostname.match(hostnamePattern)
      )
      const isSandboxMatch = !!(
        sandboxHostnamePattern && hostname.match(sandboxHostnamePattern)
      )
      ...
      console.log(
        `[Domain Check] Pattern: "${config.extensionHostnamePattern || "not set"}"`
      )
      console.log(
        `[Domain Check] Result: ${isExtensionMatch ? "EXTENSION_MATCH" : isSandboxMatch ? "SANDBOX_MATCH" : "NO_MATCH"}`
      )
    }
    await next()
  })
      ...
      getExtensionProvider: async () => {
        ...
        return {
          ...
          getBySlug: async (slug) =>
            dataSpace.extension.getExtensionBySlug(slug),
  ...
  app.use("*", async (c, next) => {
    ...
    console.warn(
      `[Auth Check] Unauthorized access attempt to ${path}. Expected Bearer token.`
    )
  ...
  app.get("/files/*", async (c) => {
    ...
    try {
      ...
      if (config.s3CustomDomain) {
        ...
        config.awsEndpoint.includes("cloudflarestorage.com") ||
        config.awsEndpoint.includes("s3.amazonaws.com")
      ) {
