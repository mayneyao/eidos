...
The physical storage structure of a Space differs depending on whether sync is enabled. When enabled, Eidos converts the single-file `db.sqlite3` into an objectified [Graft](https://graft.rs/) storage format (`.graft` directory) to achieve:
