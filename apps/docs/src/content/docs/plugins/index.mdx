...
Ordinary Eidos Lite plugins extend Spaces with custom interfaces, file editors,
document formatters, and commands. Their code runs locally in an isolated sandbox;
they install once per device and enable per Space. Standalone
[theme plugins](/plugins/workflow/#build-a-standalone-eidos-lite-theme) customize
the Lite host interface across all Spaces.
