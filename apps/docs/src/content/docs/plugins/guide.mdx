...
Ordinary Eidos Lite plugins add document formatters, command-palette actions,
and custom interfaces to a Space. Their code runs in a local, isolated sandbox.
To customize the Eidos Lite host interface itself, create a separate
[theme plugin](/plugins/workflow/#build-a-standalone-eidos-lite-theme).

## Extension capabilities

Read [Plugin concepts](/plugins/concepts/) first for the relationship between
View, File View, context, and placement. Document Views and Eidos Views are both
file opening modes; Table Views belong inside a table.

An ordinary plugin can combine any of these extension points in a single `plugin.json` manifest:
...
Declare exact HTTPS origins in `browser.networkOrigins` for `ctx.network.read`. Authenticated requests instead use manifest `connections` and host-managed credentials; see [Development workflow](/plugins/workflow/). Direct `eval`, Node.js APIs, and CDN scripts are blocked.
...
Packaging creates a `<id>-<version>.eidos-plugin` archive. This is a standardized gzip-compressed JSON envelope containing `format: 1 | 2`, full manifest metadata, and bundled module sources. Users can install it directly in Eidos Lite (**Plugins → Install plugin…**) without Node.js or an internet connection.

### 2. Publish to the Marketplace

1. Publish `<id>-<version>.eidos-plugin` as a GitHub Release asset in your repository.
2. Submit a PR to [the official registry](https://github.com/eidos-space/registry) updating `plugins.registry.json` with ID, name, description, category, repository, exact version, tag, asset filename, SHA-256, compatibility note, and the deprecated `preview: false` field. See its contribution guide or the [registry draft helper](/plugins/workflow/#prepare-a-release).
...
## Templates, debugging, and release helpers

See [Development workflow](/plugins/workflow/) for choosing a template, checking Lite/CLI compatibility, debugging, testing, and generating checksums and registry entry drafts.
