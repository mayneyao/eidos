...
<FileTree>
  - my-space/ - .eidos/ - db.sqlite3 (数据库文件) - files/ (静态资源)
</FileTree>

#### 已启用同步

<FileTree>
  - my-space/ - .eidos/ - .graft/ (VFS 存储目录) - files/ (静态资源)
</FileTree>
...
- **eidos.space (官方托管)**：零配置，登录账号即可使用。数据存储在官方提供的加密 S3 存储桶中。
- **自定义 S3 兼容服务**：使用你自己的 S3 存储（AWS S3, Cloudflare R2, MinIO 等），完全掌控数据。

:::note[Spark License 权益]
**自定义同步供应商**是 **Spark License** 持有者的专享功能。免费用户只能使用官方 eidos.space 同步。
