...
`@eidos.space/eidos-file-ui@2.0.1` 是已发布的 Eidos UI 1.0 React
...
`EidosStandardView` 只使用有界 Runtime 结果渲染标准 Grid、Gallery、Kanban 与
Calendar。未知 namespaced view type 会继续无损保存；在宿主提供隔离 renderer
之前显示 unsupported 状态。
