...
`.base` 文件是一个 SQLite 3 数据库，其中包含少量元数据表，以及一个或多个用户数据表。本文定义公开、稳定的 Base 格式 v1 契约。
...
读取方应同时检查 SQLite 文件头和 Base 元数据。仅仅使用 `.base` 扩展名，并不能让普通 SQLite 数据库成为 Base。
...
以 `eidos__` 开头的名称由 Base 元数据保留。用户数据表必须登记在 `eidos__tables` 中；不要只扫描表名前缀来识别它们。
...
File 值保存引用，通常是相对于所在 Space 的规范化路径；Base cell 不会嵌入附件二进制内容。

## Formula 与 Lookup

Formula 定义保存在字段元数据中，由 Base runtime 编译为受约束的 SQLite 表达式。Lookup 沿 Link 字段读取目标字段，并应用 `first`、`values`、`count`、`sum`、`average`、`min` 或 `max` 聚合。
...
你可以用 SQLite 工具直接检查 Base 值。应用写入应优先使用 `@eidos.space/base`：它会验证标识符与元数据、规范化 JSON 值、保持关联元数据一致，并用事务处理多步骤操作。

其他工具若直接编辑存储记录，必须保持 `_id` 唯一并写入合法字段编码。Schema 修改应通过 runtime 完成，而不是直接执行 `ALTER TABLE`，因为物理列与 `eidos__columns` 共同构成公开契约。

## 相关指南

- [构建 Base 编辑工具](#/docs/runtime)
