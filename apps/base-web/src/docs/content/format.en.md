...
A `.base` file is a SQLite 3 database with a small set of metadata tables and one or more user tables. This page defines the public Base format v1 contract.
...
| MIME type        | `application/vnd.eidos.base+sqlite3` |
| `format`         | `eidos-base`                         |
| `format_version` | `1`                                  |
| `schema_version` | `1`                                  |

Consumers should check both the SQLite header and Base metadata. A SQLite database is not a Base file merely because it uses the `.base` extension.
...
Names beginning with `eidos__` are reserved for Base metadata. User tables are registered in `eidos__tables`; do not discover them by scanning table-name prefixes alone.
...
File values are references, usually normalized paths relative to the surrounding Space. A Base file does not embed attachment bytes in the cell.

## Formula and Lookup

Formula definitions live in field metadata and are compiled into safe SQLite expressions by the Base runtime. Lookup traverses a Link field, reads a target field, and applies one of these aggregates: `first`, `values`, `count`, `sum`, `average`, `min`, or `max`.
...
SQLite tools can inspect Base values directly. For application writes, prefer `@eidos.space/base`: it validates identifiers and metadata, normalizes JSON values, keeps related metadata consistent, and wraps multi-step operations in transactions.
...
- [Build a Base editor](#/docs/runtime)
- [Build custom views](#/docs/custom-views)
- [Try the Base Web Editor](#/)
