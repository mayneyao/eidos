...
Semantic search allows you to find content based on _meaning_ rather than exact keyword matches. By converting text into vector embeddings, you can search with natural language queries like "machine learning tutorials" and find relevant items even if they don't contain those exact words.
...
<Aside type="note">
  If you followed the [Hacker News ingestion
  guide](/how-to/interact-with-desktop-app-via-api/), your table already has a
  `Content` field that you can enable embedding on.
</Aside>

<Aside type="note">
  Only **Text** fields support embedding. Title, URL, and other field types
  cannot be embedded.
</Aside>
...
<Aside type="danger">
  Resetting vectors requires re-processing all data. For large tables, this may
