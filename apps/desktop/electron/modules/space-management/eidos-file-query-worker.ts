import { parentPort } from "node:worker_threads"

import type {
  EidosFileQueryWorkerRequest,
  EidosFileQueryWorkerResponse,
} from "./eidos-file-query-worker-protocol"
import { EidosFileQueryRuntimeCache } from "./eidos-file-query-runtime-cache"

if (!parentPort)
  throw new Error("Eidos File query worker requires a parent port")
const port = parentPort
const cache = new EidosFileQueryRuntimeCache()

function errorResponse(
  request: EidosFileQueryWorkerRequest,
  error: unknown
): EidosFileQueryWorkerResponse {
  return {
    id: request.id,
    ok: false,
    name: error instanceof Error ? error.name : "Error",
    message: error instanceof Error ? error.message : String(error),
  }
}

function run(
  request: EidosFileQueryWorkerRequest
): EidosFileQueryWorkerResponse {
  try {
    const base = cache.get(request.filePath)
    if (request.operation === "page") {
      const { offset, limit, query, totalHint, cursor, projection } =
        request.options
      return {
        id: request.id,
        ok: true,
        operation: "page",
        page: base.getRowPage(
          request.tableId,
          offset,
          limit,
          query,
          totalHint,
          cursor,
          projection
        ),
      }
    }
    if (request.operation === "row") {
      return {
        id: request.id,
        ok: true,
        operation: "row",
        row: base.getRow(request.tableId, request.rowId),
      }
    }
    if (request.operation === "column-stats") {
      return {
        id: request.id,
        ok: true,
        operation: "column-stats",
        stats: base.calculateColumnStats(
          request.tableId,
          request.configs,
          request.query
        ),
      }
    }
    return {
      id: request.id,
      ok: true,
      operation: "group-counts",
      counts: base.countRowsByField(
        request.tableId,
        request.columnName,
        request.query
      ),
    }
  } catch (error) {
    return errorResponse(request, error)
  }
}

port.on("message", (request: EidosFileQueryWorkerRequest) => {
  port.postMessage(run(request))
})

const close = () => cache.close()
port.once("close", close)
process.once("exit", close)
