...
describe("EidosFileQueryWorkerRunner", () => {
  beforeEach(() => {
  ...
  it("reuses one worker per Space and routes responses by request ID", async () => {
    ...
    const runner = new EidosFileQueryWorkerRunner(lifecycle)
    const pagePromise = runner.page("/space", "/space/tasks.eidos", "tasks", {
      offset: 0,
      limit: 50,
      projection: {
    ...
    const rowPromise = runner.row("/space", "/space/tasks.eidos", "tasks", "1")
  ...
  it("runs concurrent Space reads across a bounded worker pool", async () => {
    ...
    const pages = Array.from({ length: 4 }, (_, offset) =>
      runner.page("/space", "/space/tasks.eidos", "tasks", {
        offset,
        limit: 1,
        totalHint: 4,
      })
  ...
  it("coalesces identical concurrent reads into one worker request", async () => {
    ...
    const first = runner.page("/space", "/space/tasks.eidos", "tasks", options)
    const second = runner.page("/space", "/space/tasks.eidos", "tasks", options)
    ...
    const next = runner.page("/space", "/space/tasks.eidos", "tasks", options)
    expect(harness.workers).toHaveLength(1)
  ...
  it("restores worker error names from query failures", async () => {
    ...
    const runner = new EidosFileQueryWorkerRunner(lifecycle)
    const pagePromise = runner.page("/space", "/space/tasks.eidos", "missing", {
      offset: 0,
      limit: 50,
    })
    const worker = harness.workers[0]
  ...
  it("rejects every pooled query when the Space lifecycle closes", async () => {
    ...
    for (const pagePromise of pagePromises) {
      await expect(pagePromise).rejects.toMatchObject({
        name: "EidosFileQueryClosedError",
      })
    }
    expect(harness.workers).toHaveLength(2)
    for (const worker of harness.workers) {
