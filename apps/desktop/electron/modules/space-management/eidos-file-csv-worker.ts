...
if (!parentPort) throw new Error("Eidos File CSV worker requires a parent port")
const port = parentPort

function reportProgress(
  progress: EidosFileCsvWorkerProgress["progress"]
): void {
  port.postMessage({
    type: "progress",
    progress,
  } satisfies EidosFileCsvWorkerProgress)
}
...
async function requireUnchangedFile(
  filePath: string,
  expected: EidosFileCsvFileFingerprint
): Promise<void> {
...
function issues(malformedRows: number, inconsistentRows: number) {
  const result: EidosFileCsvImportIssue[] = []
  if (malformedRows > 0) {
    result.push({
      code: "malformed-row",
      count: malformedRows,
      ...
      if (inferenceRows.length < EIDOS_FILE_CSV_INFERENCE_ROW_COUNT) {
        ...
        if (inferenceRows.length === EIDOS_FILE_CSV_INFERENCE_ROW_COUNT) {
          ...
          for (let index = 0; index < inferenceRows.length; index += 1) {
            eidosFileCsvRowToEidosFileRow(inferenceRows[index], index + 2, plan)
          }
        }
        return
      }
...
async function importCsv(
  request: EidosFileCsvWorkerRequest & { operation: "import" }
) {
  ...
  try {
    ...
    try {
      ...
      } satisfies EidosFileCsvImportResult
    } catch (error) {
      try {
...
async function exportCsv(
  request: EidosFileCsvWorkerRequest & { operation: "export" }
) {
  return exportEidosFileCsvToFile({
...
void run(workerData as EidosFileCsvWorkerRequest).then((response) => {
