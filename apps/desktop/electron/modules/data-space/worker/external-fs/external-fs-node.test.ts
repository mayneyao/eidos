...
describe("NodeExternalFileSystem", () => {
  ...
  describe("readdir", () => {
    it("should return file names when withFileTypes is false", async () => {
      ...
      const fileSystem = new NodeExternalFileSystem(
        async (fsPath: string) => {
    ...
    it("should return IDirectoryEntry with virtual paths (~/) when withFileTypes is true", async () => {
      ...
      await fs.writeFile(
        path.join(projectRoot, "subdir", "nested.txt"),
        "nested"
      )
    ...
    it.skip("should return IDirectoryEntry with virtual paths (@/) when reading mounted folder", async () => {
      ...
      const result = await fileSystem.readdir("@/music", {
        withFileTypes: true,
      })
    ...
    it.skip("should handle recursive directory reading", async () => {
      ...
      await fs.mkdir(path.join(projectRoot, "level1", "level2"), {
        recursive: true,
      })
      await fs.writeFile(
        path.join(projectRoot, "level1", "level2", "deep.txt"),
        "deep"
      )
    ...
    it("should throw error when path cannot be resolved", async () => {
      ...
      await expect(fileSystem.readdir("~/nonexistent")).rejects.toThrow(
        "Cannot resolve path: ~/nonexistent"
      )
    })
  })
  ...
  describe("mkdir", () => {
    ...
    it("should create nested directories with recursive option", async () => {
      ...
      const stats = await fs.stat(
        path.join(mountDir, "nested", "deep", "folder")
      )
      expect(stats.isDirectory()).toBe(true)
    })
    ...
    it("should throw error when path cannot be resolved", async () => {
      ...
      await expect(fileSystem.mkdir("@/nonexistent/folder")).rejects.toThrow(
        "Cannot resolve path: @/nonexistent/folder"
      )
    })
  })
  ...
  describe("direntToEntry path conversion", () => {
    it.skip("should convert absolute paths to virtual paths correctly", async () => {
      ...
      await fs.mkdir(path.join(projectRoot, "subdir"), { recursive: true })
      await fs.writeFile(
        path.join(projectRoot, "subdir", "file.txt"),
        "content"
      )
  ...
  describe("readFile", () => {
    ...
    it("should throw error when file does not exist", async () => {
      ...
      await expect(
        fileSystem.readFile("~/nonexistent.txt", "utf8")
      ).rejects.toThrow()
    })
    ...
    it("should throw error when path cannot be resolved", async () => {
      ...
      await expect(fileSystem.readFile("~/test.txt", "utf8")).rejects.toThrow(
        "Cannot resolve path: ~/test.txt"
      )
    })
  })
  ...
  describe("writeFile", () => {
    ...
    it("should write text file with encoding", async () => {
      ...
      const readContent = await fs.readFile(
        path.join(projectRoot, "test.txt"),
        "utf8"
      )
      expect(readContent).toBe(content)
    })
    ...
    it("should write binary file with Uint8Array", async () => {
      ...
      expect(Array.from(readData)).toEqual([
        0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a,
      ])
    })
    ...
    it("should write file with options", async () => {
      ...
      await fileSystem.writeFile("~/options.txt", content, {
        encoding: "utf8",
        mode: 0o644,
      })

      const readContent = await fs.readFile(
        path.join(projectRoot, "options.txt"),
        "utf8"
      )
      expect(readContent).toBe(content)
    })
    ...
    it("should throw error when path cannot be resolved", async () => {
      ...
      ).rejects.toThrow("Cannot resolve path: ~/test.txt")
    })
    ...
    it("should overwrite existing file", async () => {
      ...
      await fs.writeFile(
        path.join(projectRoot, "overwrite.txt"),
        initialContent
      )
  ...
  describe("stat", () => {
    ...
    it("should throw error when path cannot be resolved", async () => {
      ...
      await expect(fileSystem.stat("~/test.txt")).rejects.toThrow(
        "Cannot resolve path: ~/test.txt"
      )
    })
