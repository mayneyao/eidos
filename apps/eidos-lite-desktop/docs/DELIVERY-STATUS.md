...
| Whole-Space Sync        | Private-preview ready     | Real staging push/clone/pull and divergence acceptance is recorded in [Operations](./OPERATIONS.md); reviewed merge uses the published Graft SDK 0.3.17           |
...
- The reviewed merge source integration passes one real local-Graft dual-client
  ...
  and a transient filesystem Remote failure. Graft SDK 0.3.17 is pinned in the
  lockfile, and the real merge flow passed against the published registry
  package.

- Lite source suite: 107 files and 577 tests passed; 186 tests were explicitly
...
- Published Graft SDK integration: 14 passed, covering whole-Space push/clone,
  ...
  divergence analysis. The published Graft 0.3.17 merge E2E also passed both
...
- Explicit performance load: 16 passed. Explorer with 1,000 entries was 15.7
  ms; a stable change in a 10,000-entry watcher was 0.4 ms; 10 MiB and 100 MiB
  native opens were 103.5 ms and 170.7 ms; the canonical 100,000-row first page
...
- The rebuilt 0.1.12 unsigned staging package passed the packaged smoke with
  zero console errors and embedded Graft SDK 0.3.17. The accepted isolated
