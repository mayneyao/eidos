...
describe("Eidos Lite package identity", () => {
  ...
  it("keeps both macOS packaged smoke architectures in the Lite-only gate", async () => {
    ...
    expect(workflow).toContain("name: Observe fresh packaged smoke")
    expect(workflow).toContain("EIDOS_LITE_SMOKE_PERFORMANCE_POLICY: observe")
    expect(workflow).toContain("name: Enforce packaged performance")
  ...
  it("ships Lite through an isolated release and update channel", async () => {
    ...
    expect(workflow).toContain("Install native FPM on Linux arm64")
    expect(workflow).toContain("sudo gem install --no-document fpm -v 1.15.1")
    expect(workflow).toContain("USE_SYSTEM_FPM:")
    ...
    expect(workflow).not.toContain("Enforce packaged release performance")
