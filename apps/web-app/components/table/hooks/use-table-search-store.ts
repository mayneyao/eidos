...
export const createTableSearchStore = () =>
  create<TableSearchState>((set) => ({
    searchQuery: "",
    showSearch: false,
    searchResults: [],
    ...
    setSearchResults: (results, startIndex) =>
      set((state) => {
    ...
    setCurrentSearchIndex: (value) =>
      set((state) => ({
        currentSearchIndex:
          typeof value === "function" ? value(state.currentSearchIndex) : value,
      })),
    ...
    setIsSemanticSearchActive: (active) =>
      set({ isSemanticSearchActive: active }),
    setIsSemanticSearching: (searching) =>
      set({ isSemanticSearching: searching }),
    setSemanticSearchResult: (result) =>
      set({
        semanticSearchResult: result,
        semanticSearchSelectedIndex: -1,
      }),
    setSemanticSearchSelectedIndex: (index) =>
      set((state) => ({
        semanticSearchSelectedIndex:
          typeof index === "function"
            ? index(state.semanticSearchSelectedIndex)
            : index,
      })),
  }))
