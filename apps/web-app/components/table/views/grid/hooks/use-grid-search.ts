...
export const useGridSearch = (
    ...
    return searchResults.map((result: any) => {
      if (result && result.matches && result.row) {
        return result.matches
          .filter((match: any) => {
            return showColumns.some(
              (col) => col.table_column_name === match.column
            )
          })
          .map(
            (match: any) =>
          ...
          .sort(
            (a: FormattedResult, b: FormattedResult) =>
  ...
  const formattedSearchResults = useMemo(() => {
    return groupedResults
      .flat()
      .map((result) => [result.columnIndex, result.rowIndex] as Item)
  }, [groupedResults])
      ...
      if (direction === "next") {
        ...
        if (!reverseSearch()) {
          setCurrentSearchIndex((prev: number) => {
            ...
            setCellIndex(
              groupedResults[newIndex] ? groupedResults[newIndex].length - 1 : 0
            )
            return newIndex
          })
        }
  ...
  const _currentCell = useMemo(() => {
    return currentCell
      ? ([currentCell.columnIndex, currentCell.rowIndex] as Item)
      : undefined
  }, [currentCell])
