...
export const useKanbanViewData = (view: IView) => {
  ...
  const { nameRawIdMap, fieldRawColumnNameFieldMap, uiColumns } = useUiColumns(
    tableName,
    space
  )
  ...
  const fetchStatusCounts = useCallback(async () => {
    ...
    try {
      ...
      if (groupByFieldInstance?.type === FieldType.Select) {
        ...
        const options = (groupByFieldInstance as IField<SelectProperty>)
          .property.options
        ...
        const nullStatus = counts.find(
          (count) => count.status === NULL_STATUS
        ) || { count: 0 }
        if (
          !statusCountsWithOptions.some(
            (item) => item.status === NULL_STATUS
          ) &&
          nullStatus.count > 0
        ) {
  ...
  const updateItemStatus = async (itemId: string, newStatus: string) => {
    ...
    setItems((prevItems) =>
      prevItems.map((item) =>
        item.id === itemId
          ? { ...item, [groupByField]: newStatus, status: newStatus }
          : item
      )
    )
    await sqlite?._table(tableId).rows.update(
      itemId,
      { [groupByField]: newStatus === NULL_STATUS ? null : newStatus },
      {
        useFieldId: true,
      }
    )
  ...
  useEffect(() => {
    if (sqlite && nameRawIdMap.size && tableName) {
      setLoading(true)
      Promise.all([sqlite.sql2`${sql}`, fetchStatusCounts()]).then(([data]) => {
          ...
          setItems((prevItems) =>
            prevItems.map((item) =>
              item.id === _new._id
                ? {
                    ...item,
                    [groupByField]: _new[groupByField],
