...
export function EidosFileViewMenu({
  fields,
  hiddenFields,
  visibleSystemFields,
  disabled,
  ...
  const regularFields = fields.filter(
    (field) => !isOptionalEidosFileSystemField(field)
  )
  const systemFields = fields.filter(isOptionalEidosFileSystemField)
  const toggle = (field: EidosFileFieldInfo, visible: boolean) => {
  ...
  const fieldItem = (field: EidosFileFieldInfo, system = false) => (
    <DropdownMenuCheckboxItem
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          aria-label="Choose visible Eidos File fields"
          title="Fields"
          disabled={disabled}
        >
          <Columns3 className="h-3.5 w-3.5" />
          <span className="eidos-file-workbar-action-label">Fields</span>
        </Button>
      </DropdownMenuTrigger>
