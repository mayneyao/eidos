...
import { drawImage } from "@/components/table/views/grid/cells/helper"
...
export function baseFileDisplayData(paths: readonly string[]): string[] {
  return paths.map((path) => {
    if (/^(?:https?:|data:)/i.test(path)) return getFilePreviewImage(path)
...
function BaseFileCard({
  ...
  const image = getFileType(path) === "image"
  return (
    <div
...
> = ({ value: cell, onChange }) => {
  ...
  const items = cell.data.paths.map((path, index) => ({
    id: `${path}:${index}`,
    path,
    preview: cell.data.displayData[index] ?? getFilePreviewImage(path),
    index,
  }))
