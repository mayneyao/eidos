...
export type BaseEmptyStateTemplate = "blank" | "tasks"
...
export function BaseEmptyState({
  ...
  disabled?: boolean
  creatingTemplate?: BaseEmptyStateTemplate | null
  templateError?: {
    template: BaseEmptyStateTemplate
    message: string
  } | null
  importAction: ReactNode
  onCreateTemplate: (template: BaseEmptyStateTemplate) => void
}) {
  return (
    <section
      className="flex min-h-0 flex-1 flex-col items-center justify-center px-6 py-10 text-center"
      aria-labelledby="base-empty-state-title"
      aria-describedby="base-empty-state-description"
    >
      <Table2 className="mb-3 h-5 w-5 text-muted-foreground" />
      <h2 id="base-empty-state-title" className="text-sm font-medium">
        Start this Base
      </h2>
      <p
        id="base-empty-state-description"
      ...
      <div
        ...
        aria-label="Ways to create the first Base table"
        role="group"
      >
        <Button
          type="button"
