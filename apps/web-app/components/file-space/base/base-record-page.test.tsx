...
describe("BaseRecordPage", () => {
  ...
  it("keeps Base context in a compact workbar and edits in the page surface", async () => {
    const onBack = vi.fn()
    ...
    expect(container.textContent).toContain("tasks.base")
    ...
    expect(
      container.querySelector('[data-base-record-layout="page"]')
    ).not.toBeNull()
  ...
  it("offers a non-modal escape when the requested table is gone", async () => {
    ...
    await act(async () => {
      Array.from(container.querySelectorAll("button"))
        .find((button) => button.textContent === "Back to Base")
        ?.click()
    })
    expect(onBack).toHaveBeenCalledOnce()
  })
