...
} from "./base-record-inspector"

type BaseRecordPageProps = Omit<
...
export function BaseRecordPage({
  baseName,
  tableName,
  error,
  onBack,
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 justify-center overflow-hidden bg-muted/10 px-[clamp(0px,3vw,40px)]">
        <BaseRecordInspector variant="page" {...inspectorProps} />
      </div>
    </div>
  )
}

export function BaseRecordUnavailable({
  baseName,
  message,
  onBack,
}: {
  ...
  return (
    <div
      ...
      data-base-record-page
    >
      <BaseRecordWorkbar baseName={baseName} onBack={onBack} />
