...
const fields = ["title", "status", "owner"].map(
  (tableColumnName, index): BaseFieldInfo => ({
...
describe("Base view layout commands", () => {
  it("orders and freezes columns from the persisted view", () => {
    expect(orderedBaseFields(fields, view).map((field) => field.name)).toEqual([
      "owner",
      "title",
      "status",
    ])
    expect(baseViewFreezeColumns(view, fields.length)).toBe(2)
    expect(
      baseViewFreezeColumns(
