...
export function BaseCreatePopover({
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(null)
    try {
      const title = normalizedName.slice(0, -".base".length)
      await onCreate(normalizedName, baseOptionsForTemplate(title, template))
      onOpenChange(false)
    } catch (createError) {
      setError(
      ...
      >
        <form onSubmit={(event) => void submit(event)}>
          <div className="border-b px-4 py-3">
            <h2 className="text-sm font-semibold">New Base</h2>
          ...
          <div className="flex items-center justify-end gap-2 border-t px-4 py-2.5">
            ...
            <Button type="submit" disabled={submitting || !normalizedName}>
              {submitting ? "Creating…" : "Create Base"}
            </Button>
          </div>
        </form>
      </PopoverContent>
