...
export const useDocPropertyTypes = () => {
  ...
  const customPropertyTypes = useCallback(() => {
    ...
    return propertyTypes.filter(
      (prop) => !reservedProperties.includes(prop.name)
    )
  }, [propertyTypes])

  const getPropertyType = useCallback(
    (propertyName: string): FieldType | undefined => {
  ...
  const getPropertyTypes = useCallback(async () => {
    ...
    const { setLoading, setError, setPropertyTypes } =
      useDocPropertyStore.getState()
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to get property types"
  ...
  const changePropertyType = useCallback(
    async (propertyName: string, newType: FieldType) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Failed to change property type"
