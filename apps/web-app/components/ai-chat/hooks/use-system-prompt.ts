...
export const useAdditionalData = (contextNodes: ITreeNode[] = []) => {
  const { space } = useCurrentPathInfo()
  const currentNode = useCurrentNode()
  const [_map, { set, reset, get }] = useMap<string, string>()
  const [_tableMap, { set: setTable, reset: resetTable, get: getTable }] =
    useMap<string, string>()
  const [
    _extensionMap,
    { set: setExtension, reset: resetExtension, get: getExtension },
  ] = useMap<string, { slug: string; code: string }>()
  const [
    _pathDocMap,
    ...
    () =>
      contextNodes
        .filter((node) => {
          ...
          return (
            isPathNode &&
            allowedTextExtensions.some((ext) => fileName.endsWith(ext))
          )
        })
        .map((node) => node.id),
    [contextNodes]
  ...
  updated_at?: string
          ...
          const schemaText = columns.map((col) => `${col.name}`).join("\n")
  ...
  <additional_data>
  <attached_docs>
  ${Array.from(_map.entries())
    .map(
      ([key, value]) =>
        `<doc id="${key}" title="${contextNodes.find((node) => node.id === key)?.name}">\n${value}\n</doc>`
    )
    .join("\n")}
  </attached_docs>
  <attached_tables>
  ${Array.from(_tableMap.entries())
    .map(
      ([tableName, schema]) =>
        `<table id="${getTableIdByRawTableName(tableName)}" name="${tableName}" title="${contextNodes.find((node) => getRawTableNameById(node.id) === tableName)?.name}">\n${schema}\n</table>`
    )
    .join("\n")}
  </attached_tables>
  <attached_extensions>
  ${Array.from(_extensionMap.entries())
    .map(
      ([extensionId, extensionData]) =>
        `<extension id="${extensionId}" slug="${extensionData.slug}">\n${extensionData.code}\n</extension>`
    )
    .join("\n")}
  </attached_extensions>
  <attached_path_docs>
  ${Array.from(_pathDocMap.entries())
    .map(
      ([path, content]) =>
        `<doc path="${path}" title="${contextNodes.find((node) => node.id === path)?.name}">\n${content}\n</doc>`
    )
    .join("\n")}
  </attached_path_docs>
  </additional_data>
...
export const useSystemPrompt = (contextNodes: ITreeNode[] = []) => {
  ...
  }, [extensionData.bindings, extensionData.code, basePrompt, getRemixPrompt])
