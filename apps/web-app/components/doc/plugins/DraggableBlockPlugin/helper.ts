  ...
  while (currentElement && currentElement !== anchorElem) {
    if (
      ...
      (currentElement.hasAttribute("data-lexical-decorator") &&
        currentElement.getAttribute("data-type") !== "mention")
    ) {
...
export function setTargetLine(
  ...
  if (targetBlockElem.tagName === "LI") {
    ...
    if (
      !isFirstItem ||
      mouseY - targetBlockElemTop > targetBlockElemHeight / 2
    ) {
  ...
  const left =
    targetBlockElemLeft - anchorElem.getBoundingClientRect().left + padding
