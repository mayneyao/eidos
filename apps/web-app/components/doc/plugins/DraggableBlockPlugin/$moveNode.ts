...
export function $moveNode(
  ...
  if (targetNode.__type !== "listitem" && draggedNode.__type === "listitem") {
    ...
    const listType: ListType =
      parentList?.__type === "list" ? parentList.getListType() : "bullet"
  ...
  if (targetNode.__type === "list" || targetNode.__type === "listitem") {
    if ($isListItemNode(targetNode) && event) {
      ...
      if (targetElement) {
        ...
        if (
          (!isFirstItem || event.clientY - top > height / 2) &&
          ...
          if (toBeMovedNode.__type === "listitem") {
            ...
            ;(toBeMovedNode as ListItemNode).setIndent(
              (toBeMovedNode as ListItemNode).getIndent() + 1
            )
          } else {
