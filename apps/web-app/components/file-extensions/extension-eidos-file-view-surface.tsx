...
import type { FileExtensionOpenEidosFileViewResult } from "@/apps/desktop/electron/modules/file-extensions/types"
...
function extensionEidosFilePage(page: EidosFileRowPage) {
...
export function ExtensionEidosFileViewSurface({
  extension,
  filePath,
  table,
  view,
  ...
  extension: FileExtensionEidosFileView
  filePath: string
  ...
  const [session, setSession] =
    useState<FileExtensionOpenEidosFileViewResult | null>(null)
  ...
  const context = useMemo<ExtensionEidosFileViewContextSnapshot>(() => {
  ...
  useEffect(() => {
    ...
    port.postMessage({ type: "eidos-file-context-changed", context })
  }, [context])

  useEffect(() => {
    const spaceId = currentSpace?.id
    if (!spaceId || !window.eidos?.fileExtensions?.openEidosFileView) {
      setLoading(false)
      setError(
        "Extension Eidos File views are available in Desktop file Spaces."
      )
      return
    }
    let cancelled = false
      ...
      .catch((openError: unknown) => {
        ...
        setError(
          openError instanceof Error
            ? openError.message
            : "Unable to open the extension Eidos File view."
        )
      })
      .finally(() => {
    ...
    const listenerId = window.eidos.on(
      ...
      (_event: unknown, payload: unknown) => {
        ...
        if ("status" in payload && payload.status === "ready") {
          ...
          setActivated(false)
          setLoading(false)
          setError("Fix the extension source to reload this Eidos File view.")
        }
      }
    )
    return () => {
      ...
      channel.port1.addEventListener("message", (event) => {
        ...
        if (message?.type === "ready") {
          ...
          channel.port1.postMessage({
            ...
            eidosFileViewId: session.eidosFileViewId,
            viewId: view.id,
            context,
        ...
        if (message.generation !== session.generation) {
          channel.port1.postMessage({
            ...
            ok: false,
            error: { message: "The extension Eidos File view is stale." },
          })
          return
        }
        void loadPage(message.offset, message.limit).then(
          (page) => {
            channel.port1.postMessage({
              ...
              requestId: message.requestId,
              ok: true,
              page: extensionEidosFilePage(page),
            })
          },
          (pageError: unknown) => {
            channel.port1.postMessage({
              ...
              error: {
                message:
                  pageError instanceof Error
                    ? pageError.message
                    : "Unable to load Eidos File rows.",
              },
            })
          }
        )
  ...
  if (loading) {
    return (
      <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
        ...
        Opening extension Eidos File view…
      </div>
    )
  }
      ...
      <div className="flex h-full items-center justify-center p-6">
        <div className="max-w-md text-center">
          ...
          <p className="text-sm font-medium">
            Extension Eidos File view unavailable
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            {error ?? "The extension Eidos File view could not be initialized."}
          </p>
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col bg-background">
      <iframe
        key={`${session.eidosFileViewId}:${session.generation}`}
