...
export const useExtensionMarketplace = ({
  ...
  // check update
  const checkUpdate =
    useCallback(async (): Promise<LatestVersionResponse | null> => {
      if (!isUuid(script.id)) {
        return null
      }
  ...
  const submitExtension = useCallback(async () => {
    ...
    try {
      ...
      const payload = {
        ...
        type: script.meta?.type
          ? `${script.type}/${script.meta.type}`
          : script.type,
  ...
  const publishNewVersion = useCallback(async () => {
    if (!script?.marketplace_id) {
      toast({
        ...
        description:
          "Missing extension ID. Has the extension been submitted initially?",
      })
      return
    }
    if (!hasAuth) {
