...
`
  for (const [key, value] of Object.entries(bindings ?? {})) {
    const fields = await sqlite?.column.list({
      table_name: getRawTableNameById(value.value),
    })
    bindingsPrompt += `### ${key}\n`
...
export const useRemixPrompt = () => {
  ...
  const getRemixPrompt = useCallback(
    async (
      ...
      const {
        ...
        useSdk,
        useUiGuide,
        useCodePatchingMode = true,
      } = options ?? {}
      let bindingsPrompt = await getBindingsPrompt(bindings ?? {}, sqlite)
      let prompt = defaultPrompt
      let res = prompt.replace("{{bindings}}", bindingsPrompt).replace(
        "{{userCode}}",
        `
            <user-code>
            \`\`\`jsx
