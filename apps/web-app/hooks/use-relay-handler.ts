...
export const useRelayHandler = () => {
    ...
    async (targetRelayId?: string) => {
      ...
      try {
        for (const relay of relaysToProcess) {
          ...
          // 2. Load the script
          const script = (await sqlite.extension.get(relay.handlerScriptId)) as
            | IExtension<RelayHandlerMeta>
            | undefined
          if (!script || !script.enabled) {
            console.log(
              `[RelayHandler] Bound script not found or disabled for relay ${relay.id}:`,
              relay.handlerScriptId
            )
            continue
          }
          ...
          while (true) {
            ...
            console.log(
              `[RelayHandler] Processing batch of ${messages.length} messages for ${relay.id}...`
            )
            ...
            try {
              ...
              const finalAcked = Array.from(
                new Set([...ackedIds, ...implicitlyAcked])
              )
              ...
              console.error(
                `[RelayHandler] Script execution failed for relay ${relay.id}:`,
                scriptError
              )
  ...
  useEffect(() => {
    ...
    const handler = (event: any, data?: any) => {
      ...
      console.log(
        `[RelayHandler] Received relay-messages-ready for relayId: ${relayId}`
      )
