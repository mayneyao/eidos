...
const eidosFile = createEidosFile(output, {
...
for (let offset = 0; offset < rows.length; offset += 250) {
  eidosFile.insertImportedRows("projects", rows.slice(offset, offset + 250))
}
eidosFile.createView("projects", {
  name: "By status",
  type: "kanban",
...
const reopened = openEidosFile(output, { readonly: true })
try {
  const validation = reopened.info()
  if (validation.format !== "eidos-file") {
    throw new Error("Fixture did not reopen as an Eidos File")
  }
