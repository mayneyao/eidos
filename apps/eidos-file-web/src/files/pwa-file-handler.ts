...
interface RegisterPwaEidosFileHandlerOptions {
...
type LaunchSubscriber = Pick<
  RegisterPwaEidosFileHandlerOptions,
  "onOpen" | "onError"
>
 ...
 */
export function registerPwaEidosFileHandler({
  onOpen,
  onError,
  target = window as Window & PwaLaunchTarget,
}: RegisterPwaEidosFileHandlerOptions): () => void {
  ...
  if (!state) {
    ...
    queue.setConsumer(async ({ files = [] }) => {
      try {
        ...
        if (!handle) return
        const file = await openEidosFileHandle(handle)
        state!.pending.push({
          type: "open",
          file,
        })
