...
function fileHandle(name = "tasks.eidos"): FileSystemFileHandle {
...
describe("PWA Eidos File handler", () => {
  ...
  it("reports a mismatched extension and leaves the editor untouched", async () => {
    ...
    await vi.waitFor(() => {
      expect(onError).toHaveBeenCalledWith(
        expect.objectContaining({ message: expect.stringContaining(".eidos") })
      )
    })
  })
  ...
  it("feature-detects launchQueue and deactivates stale consumers", async () => {
    ...
    let consumer:
      ...
      | undefined
    const onOpen = vi.fn()
    const cleanup = registerPwaEidosFileHandler({
      onOpen,
      onError: vi.fn(),
      target: {
        launchQueue: {
