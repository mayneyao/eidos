...
class DatabaseManager {
  ...
  getConnection(
    ...
    if (this.connections.has(dbPath)) {
      ...
      console.log(
        `[DatabaseManager] Reusing existing connection (refs: ${refCount + 1})`
      )
    ...
    console.log(
      `[DatabaseManager] Creating new connection with options:`,
      options
    )
    try {
      ...
      console.log(
        `[DatabaseManager] ✓ Connection created (total: ${this.connections.size})`
      )
      return db
    } catch (error: any) {
    ...
    if (newRefCount === 0) {
      ...
      if (db) {
        try {
          ...
          console.error(
            `[DatabaseManager] Error closing connection:`,
            error.message
          )
        }
      }
  ...
  closeAll(): void {
    console.log(
      `[DatabaseManager] Closing all connections (${this.connections.size} total)`
    )
    ...
    for (const [dbPath, db] of this.connections.entries()) {
      try {
        ...
        console.error(
          `[DatabaseManager] Error closing ${dbPath}:`,
          error.message
        )
      }
    }
