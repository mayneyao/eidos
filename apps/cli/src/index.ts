...
program
  ...
  .argument(
    "[path]",
    "Path to initialize the space (defaults to current directory)"
  )
...
program
  ...
  .action(
    async (targetPath?: string, options?: { port?: string; host?: string }) => {
      await serveCommand({
        path: targetPath,
...
graftCommand
  ...
  .option(
    "-r, --remote <url>",
    "Remote graft URL (e.g., https://eidos.space/username/space.graft)"
  )
  ...
  .option(
    "--endpoint <url>",
    "S3 endpoint URL (default: https://s3.eidos.space)"
  )
  .action(
    async (
      targetPath?: string,
