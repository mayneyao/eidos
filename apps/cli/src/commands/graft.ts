 ...
 */
function applyGraftConfigToEnv(
  spacePath: string,
...
function registerGraftVFS(db: Database, graftLibPath: string): void {
  try {
    ...
    throw new Error(
      `Failed to load graft VFS extension from ${graftLibPath}: ${err.message}`
    )
  }
}
...
function getSyncCredentials(options: GraftInitOptions): SyncCredentials {
  ...
  const bucketName =
    options.bucketName || process.env.AWS_BUCKET_NAME || "eidos-sync"
  const endpoint =
    options.endpoint || process.env.AWS_ENDPOINT || "https://s3.eidos.space"
  ...
  try {
    ...
    if (options.remote) {
      ...
      if (!remoteMatch) {
        ...
        logger.info(
          "Remote URL should be like: https://eidos.space/username/space-id.graft"
        )
        process.exit(1)
      }
    ...
    if (!fs.existsSync(extensions.graft.libPath)) {
      ...
      logger.info(
        'Please run "bun run setup" to download the required extensions.'
      )
      process.exit(1)
    }
    ...
    try {
      ...
      try {
        ...
        if (space) {
          const remoteUrl =
            options.remote ||
            `https://eidos.space/${os.userInfo().username}/${remoteSpaceId}.graft`
