...
pub fn run(args: PluginArgs, human: bool) -> Result<CommandOutput> {
    let (command, directory, output) = match args.command {
        PluginCommand::Fs { .. } => {
            return Err(AppError::invalid_request(
                "Filesystem transport requires the host protocol",
            ));
        }
        PluginCommand::Create { directory } => return create(&directory),
        PluginCommand::Check(args) => ("check", args.directory, args.output),
        PluginCommand::Dev(args) => ("dev", args.directory, args.output),
        PluginCommand::Pack(args) => ("pack", args.directory, args.output),
    };
...
fn create(directory: &Path) -> Result<CommandOutput> {
    ...
    let package = json!({"name":name,"version":"0.1.0","private":true,"type":"module","scripts":{"check":"eidos plugin check","pack:plugin":"eidos plugin pack"},"devDependencies":{"@eidos.space/plugin-sdk":"^0.1.0"}});
