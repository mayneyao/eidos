# RFC：Space、Eidos File 与 Changes 的产品交互
...
`.eidos` route 现已使用 production Grid 交互层，支持按可见区域分页加载、optimistic
...
结构化查询模型，因此删除的始终是 Grid 当前展示并选中的记录。Eidos File 创建支持命名、
模板和原子发布。working Changes 与 History 也已提供 Eidos File-aware table/row inspector。
...
Eidos File 导航现在按 workbook 层级组织，不再把 table 和 view 混在同一工具栏：当前 table 的
...
现有 Eidos File actions overflow 现在也可以在系统文件管理器中显示当前 `.eidos` 文件，补齐 workbook
的文件定位路径，同时不增加新的工具栏图标或弹窗。定位失败会留在 editor 的可恢复 alert 边界内，
用户可以关闭提示并从同一个菜单重试。

Cmd+P 继续使用 Space 统一 Quick Open，不引入 Eidos File 专用弹层。当前 tab 为 `.eidos` 时，
结果顶部会增加 `Tables in <file>.eidos` 上下文分组，可按 table 名、底层 table 标识或
Eidos File 路径过滤并直接切换；普通文件结果仍在同一面板中。底部 sheets 同时支持
Ctrl+PageUp/PageDown 循环切换，保留高频键盘路径而不改变全局搜索的心智模型。

Grid、Gallery 与 Kanban 的 Record Inspector 现在都提供 `Open record in tab`。这个非模态
tab 使用稳定的 `.eidos` 路径、table ID 与 record ID URL，因此可以固定、分屏、恢复，并在
Eidos File 文件移动后继续保持目标。全页布局直接复用 Inspector 字段编辑器、附件与 relation adapter、
optimistic mutation queue 和独立 Eidos File runtime，不引入第二条记录存储链路，也不依赖旧 document
block。tab title 会跟随已加载记录；file watcher revision 会在原位刷新记录；table 或记录不存在时，
页面仍保留返回 Eidos File workbook 的可恢复路径。
...
Eidos File row mutation 现在会返回提交后的 metadata revision。Renderer 用 revision 识别延迟到达的
...
独立 Eidos File runtime 的同一个 transaction 提交；任意一行失败都会回滚整个 range。行级
...
会由独立 Eidos File runtime 在持久 query worker 中通过一次参数化聚合查询完成；Renderer 不扫描已加载
...
40px。Files/Version 分区栏与 Eidos File view workbar 使用同一 workbar token，Space footer 与 Eidos File
sheet bar 使用同一 statusbar token，跨 sidebar 和内容区共享的水平边界不再由组件各自写死。

Eidos File workbar 现在按 editor 容器而不是整个窗口自适应。内容区变窄时，action labels 会收敛为
...
file 字段现在使用从原表格交互适配出的 Eidos File 专用多附件 cell。用户可以把文件导入
...
reference browser、即时依赖/错误反馈和保存前的真实 Eidos File row sample preview。两个流程都
不打开居中弹窗。
...
maximum、color 与 label 配置会直接作用到 Grid。删除 option 会在同一 Eidos File transaction 中
...
键盘选择；Eidos File 不再把不同能力的字段压进一个无差别下拉列表。
...
公平读写语义：Gallery 分页与多个可见 Kanban 列可以同时进入；排队中的 Eidos File mutation、文件写入或
Graft restore 仍保持独占，并且不会被后到的读取越过。4 个并发可见列读取因此从 1 条串行执行通道变为
2 条有界通道，同时不会把同步 SQLite 工作搬回 Electron 主线程。重复虚拟滚动会复用已经校验的 Eidos File
...
稳定 Eidos File schema 的 Formula 编译计划现在也由上述长生命周期 runtime 按 table 缓存。Gallery 或
...
独立 Eidos File runtime 还会为 Gallery 排序前缀和 Kanban 分组+排序前缀维护可丢弃的 SQLite 查询索引。
索引跟随 view lifecycle，在被索引字段转换或删除后重建，并在旧文件以 migration 模式打开时一次性修复；
它们只加速物理表，不进入 Eidos File metadata 语义契约。可写 Eidos File lifecycle 会先创建或修复索引，再由只读
...
激活；card 内 button、link 和 input 的 pointer/keyboard 事件仍由控件自身处理。Eidos File blocking
...
内部 measurement 窗口最多保留 2,048 条，并以 512 条为分段前移；相邻窗口至少保留 75% 重叠。
相较早期 20,000 条基线，这会削减 89.8% 的单列表 measurement entries；验收机上的一次百万记录
聚焦回归组件耗时从 590ms 降到 572ms，具体 wall-clock 仍会受运行环境影响。跨段时会清理另一份按
...
layout。独立 Eidos File package 会收集搜索、嵌套筛选和排序实际读取的字段，并递归追踪 Formula/Lookup
...
隐藏空字段时，card renderer 还会在收集到 Gallery 的 6 个或 Kanban 的 4 个展示字段后立即停止读取
row，而不是先过滤完整字段列表再截断。稀疏记录仍会继续扫描到足够的非空字段或字段末尾；数字 `0` 和
布尔 `false` 继续按有效值显示。1,000 个字段且前 6 个字段均有值的隔离基准中，50,000 次选择从五轮
中位数约 2,899 ms 降到 11.8 ms；访问次数回归同时要求该场景只读取 6 个 row values。关闭隐藏空字段
后直接截取共享 layout，不读取 row values。这个边界让大字段 Eidos File 的卡片渲染成本取决于实际展示上限，
...
请求风暴，也不再要求用户重新加载整个 Eidos File。可恢复错误只留在受影响的 Gallery 或 Kanban 列内，
不会在原位重试成功后仍残留一条全局 Eidos File 错误提示。Kanban grouped-count 失败遵循相同边界：首次
...
重复的全局 Eidos File alert。

锚定式 Eidos File 创建、重命名、view 设置、Filter/Sort 和字段 Property workspace 共同遵循可恢复的
transaction 边界：保存期间不能关闭或重复提交，失败后保留当前草稿并只显示一条局部错误，父编辑器
仍会加载持久化快照，但不会再追加第二条全局 Eidos File alert。Formula 与 Lookup 编辑器也使用同一规则，
...
mutation 继续使用全局 Eidos File alert。
...
重新加载持久化 Eidos File 快照，仍会保留并显示当前 optimistic 字段值；在用户解决冲突前锁定其他字段编辑，
...
全局 Eidos File alert。Card 移动失败仍只回滚受影响的列并在局部播报恢复。Board 边界现在会串行化 card
...
Eidos File 的次级工作区现在根据 active editor 的实际可用空间响应，而不是使用整个应用窗口宽度。Grid、
...
空 Eidos File 的首次使用状态现在直接提供三条真实路径：一键创建与新建空白 Eidos File 相同的默认 table、用同一
套 schema 创建 Task tracker，或从当前空状态启动流式 CSV 导入。CSV 入口不再只藏在空工具栏中，
两个模板也不复制另一份字段定义；创建失败会把错误和 Retry 留在空状态，不会追加一条全局 Eidos File
alert。整个流程作用于当前 `.eidos` 文件，不创建教学副本，也不增加居中弹窗；需要自定义名称时，
底部 sheet `+` 仍提供完整的锚定式创建流程。

Space file route 现在按文件类型异步加载 Eidos File workspace。Markdown、普通文本、图片和未知文件不再静态
下载并解析 Grid、Gallery、Kanban 与 Formula composer；第一次打开 `.eidos` 时显示占满 editor surface
的稳定 loading 状态，模块加载失败会留在当前 tab 并支持原位 Retry，成功后的模块 Promise 会在多个
Eidos File tabs 之间复用。Files tree 会在用户悬停或用键盘聚焦 `.eidos` 文件时复用同一个 Promise 进行意图
...
同时生成 412,145-byte 的 `space-eidos-file-editor` chunk 和独立 CodeMirror dependency chunk；入口 gzip
...
等待 SQLite transaction 回滚，再解除当前 Eidos File 的 mutation lock，因此重试不会和仍在退出的
...
transaction 中通过 cursor/offset 分页读取 `.eidos`，流式写入带 UTF-8 BOM 的临时 CSV，完成后再
原子替换用户通过原生 save picker 选择的目标。进度按真实 rows 展示并可取消；失败或取消只会清理
临时文件，不会留下看似成功的截断 CSV。真实 1,205 行跨页用例已验证筛选、排序、显示值和 603 行
输出结果。

真实文件 Eidos File versioning smoke 现在会创建 Grid、Gallery 和 Kanban metadata，关闭并重开文件，
编辑行，验证 Graft row diff，恢复初始 revision，再次重开并校验 records、派生值和三种 view
layout；恢复后的仓库状态为 clean。

原生 Desktop 也已经通过同一条产品链路验收：从 UI 新建命名的 Task tracker Eidos File，编辑基础字段
和 Select cell，只 stage 该文件并创建版本，完整重启 Electron 后重开并校验记录，再制造 dirty edit，
最后从 History 恢复文件且不移动 HEAD。Graft 替换文件后，已打开的 Eidos File 会立即刷新；恢复当前
...
| Eidos File 首次使用                        | 空白表、Task tracker 与 CSV 三条路径均已接通        | v1 暂无已知缺口                                                     |
| Eidos File 默认设置与附件目录              | 按 Space 预选模板并写入可移植相对文件路径           | v1 暂无已知缺口                                                     |
| Eidos File workspace 加载                  | 文件类型级 chunk、稳定 loading 与原位 Retry         | v1 暂无已知缺口                                                     |
...
| Eidos File merge conflict 审阅             | 原生 row 审阅已验收                                 | schema/opaque conflict 按设计使用明确的 whole-file fallback         |

这是 v1 Eidos File table-view 的交付候选，并不试图把旧 document 或 extension 耦合复制进独立 package。
可移植的 File 与 URL cover 已经满足 v1 Eidos File 边界；旧 document-content 与 extension-block cover
明确保留在独立 package 之外。
Eidos File 日常编辑和配置优先使用单元格内编辑、表头菜单、锚定 Popover 和渐进披露；居中弹窗只保留
给破坏性确认或必须中断当前工作流的决策。新增交互应先参考并复用原表格已经验证过的编辑方式，
不应仅为了实现方便把字段配置、记录编辑或 view 管理改成弹窗流程。

file Space Settings 已拆分为 General、Files/Obsidian、Eidos File、Versioning 和派生 Indexes。
Eidos File 设置用与其他 Codex 风格设置一致的单层行分组，按 Space 保存新建文件时预选的 Blank/Task
tracker 模板，以及记录附件复制到 Space 根 `assets/` 或 Eidos File 同目录 `assets/` 的策略。模板仍可在
...
resolution 已通过双 Space Desktop 验收。Eidos File conflict 现在会打开独立的非模态审阅 tab：
Graft 的 row/schema/opaque artifact 会保持结构化，row 值按 Eidos File/current/incoming 字段对比，
...
Eidos File 自动 staged、双 parent merge continuation 和最终 push。

## 摘要

本 RFC 定义 Eidos 转向 file-based Markdown 文件和 `.eidos` 结构化数据文件后的产品交互模型。

UI 应该让存储模型变得清楚：

- Markdown 文件是文档。
- Eidos File 文件是结构化数据工作簿。
- Assets 是普通文件。
...
- 点击 `.eidos` 打开 Eidos File workspace，
- 点击图片/PDF 打开预览，
- folders 可以展开/收起，
- `.eidos/` 和 `.graft/` 默认隐藏。
...
1. 选择文件名，
2. 选择模板或空白，
3. 打开 Eidos File editor，
4. 创建第一张表。

## Eidos File 工作区

打开 `tasks.eidos` 应显示 Eidos File 专属 workspace：
...
Eidos File 内部对象不应该作为单独 Space 文件出现。
...
- Eidos File 文件显示 table-level diff。
...
2. `.eidos` 在 Space tree 中保持为一个文件；tables 和 views 只在 Eidos File workspace 与 Quick Open
   上下文中展示。
3. Changes 保持 path-first；选择 `.eidos` 会打开结构化 Diff tab，不在 sidebar tree 内展开生成式
   diagnostics。
...
- file tree 能区分 `.md`、`.eidos`、assets，
- Markdown editor 保存真实 `.md`，
- Eidos File workspace 打开 `tasks.eidos`，
- Changes tree 显示这三个 changed paths，
- 展开 `tasks.eidos` 显示 table changes，
- 私有 `.eidos` 运行时状态保持隐藏。
