...
| Graft 版本管理    | 本地链路验收通过 | Changes、staging、commit、diff、分页 history、文件/目录 discard、path/Space restore 和 Base inspection 已通过原生 Desktop 验收；remote sync 和 conflicts 仍待实现。 |
...
下一顺序是 remote sync/conflicts，然后是文件化扩展以及剩余的 Base/产品细化。
