...
| RFC               | 状态             | 当前实现边界                                                                                                                                        |
| ----------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| Space/Base 存储   | 实施中           | file-based 垂直切片、独立 Base runtime、Base-aware diff、真实 legacy export 和隔离 remote 验收已跑通；仍需原生 remote dogfooding 与文件化扩展。     |
...
| Graft 版本管理    | Remote 垂直切片  | 本地链路通过原生 Desktop 验收；持久 remote config/fetch/pull/push、path-first conflicts、双 parent continuation 和隔离双 Space 验收均已跑通。       |
| 产品 UX           | 实施中           | Files/Version、Diff/History、上下文 Settings、紧凑同步控件、冲突操作、正式 Base grid 和 Legacy Migration Settings 已可用；仍需原生同步 dogfooding。 |
...
下一顺序是原生 remote/conflict dogfooding，然后是文件化扩展以及剩余的 Base/产品细化。
