# RFC：Eidos Space/Eidos File 存储模型

状态：草案，Eidos File v1 Desktop 验收通过
日期：2026-07-08
负责人：Eidos
...
Markdown 派生索引、Space-root 本地 Graft 版本管理和独立 `.eidos` 格式/runtime
基础已经实现。Changes 和历史版本现已支持按选中的 SQLite path 请求 row details，
并展示 Eidos File table/row operations，而不污染普通 status refresh。独立 legacy export
runtime 已经可以规划并原子生成经过校验的 Markdown + `main.eidos` + assets Space，
...
验收；原生 Eidos File 创建/编辑/重启/版本/恢复生命周期，以及稳定 record tab 的打开、编辑、
外部刷新、分屏与重启恢复同样已经验收。文件化扩展作为独立 RFC 当前暂缓。

因此本文的 phase 顺序只描述能力依赖，不再代表当前执行顺序。已经完成的顺序是 Markdown
Space、本地 Graft、独立 Eidos File runtime、migration 和原生 remote Graft 验收。Eidos File v1
...
- Eidos File 文件是 Space 里用户可见的一等结构化数据文件。
- Eidos 命名空间下的状态放在 `.eidos/` 下，但需要区分可版本管理的 source/config 和私有运行时状态。
- Graft 为整个 Space 提供版本管理，并且能对 Eidos File 文件提供 SQLite 级别的 diff。

目标产品叙事不应该是「Eidos 把 Obsidian vault 存进一个隐藏数据库」。目标叙事应该是：

> Eidos 打开一个本地 Space，保留 Markdown 和资源文件的文件形态，新增一等的结构化 Eidos File 文件，并使用 graft 为整个空间提供版本管理。
...
存储模型应该明确表达这一点：Markdown 保持 Markdown，Eidos 的结构化数据应该成为一种一等文件格式：Eidos File。
...
- Space 仍然是一个本地文件夹。
- Markdown 文件仍然是文档资产。
- Eidos File 文件是结构化数据资产。
- Graft 为 Space 提供版本管理，并理解 Eidos File 的内部变更。
...
它可以包含 Markdown 文件、Eidos File 文件、资源文件、app 配置目录和 graft 元数据。
...
Eidos File 底层是 SQLite 数据库。产品心智上，它更接近 Excel workbook 或 Airtable base：
...
Eidos 可以直接打开一个 Eidos File 文件，将其渲染成表格和视图，并通过 graft 提供表格级版本 diff。
...
Graft 应该保持通用，不需要写死 Eidos 专属逻辑来理解 `.eidos` 文件是 SQLite。它应该基于文件检测和配置的 tracking rules 工作。
...
但在目标 Space/Eidos File 模型中，graft 应该追踪用户可见的 Space 资产和选定的 Eidos 项目文件，并忽略私有运行态。
...
> Eidos File 是 Space 里的一个文件，但 Eidos 能检查它内部的 SQLite 变更。

目标模型下，UI 不应该把 `.eidos/db.sqlite3` 暴露成主要用户资产。

## Eidos File 文件格式要求

`.eidos` 文件应该是合法 SQLite 数据库。
...
`.eidos` 是 Eidos File 文件的规范扩展名。Eidos 仍然同时检查 SQLite 文件头与元数据，不会仅凭扩展名判断文件身份。
...
- Eidos File 专属的表、视图、字段和关系迁移到 `.eidos` 文件里。
- `.eidos/db.sqlite3` 应该收缩为 workspace 私有元数据、缓存、本地设置和迁移支持。
- 文件元数据需要重新考虑。如果文件是普通 Space 文件，它的文件系统路径可以是 canonical identity。如果它是 Eidos File 内部引用的附件，Eidos File 可以通过相对路径或托管 payload ID 引用它。

`eidos__tree` 需要单独决策：

- 对 Space 文件来说，真实文件系统树应该是 canonical tree。
- 对 Eidos File 内部来说，tables/views 可以在 Eidos File 内部拥有自己的排序和分组。
...
- 保留普通资源文件和链接，
- 将 `.eidos` 文件作为 Eidos 专属结构化资产加入 Space，
- 只有在启用版本管理时才添加 `.graft/`。
...
### Phase 1：Eidos File 文件支持

在保留当前 `.eidos/db.sqlite3` 模型的同时，新增创建和打开 `.eidos` 文件的能力。
...
- 列出 `.eidos` 内部 tables，
- 编辑 `.eidos` 内表格数据，
- 检测 `.eidos` 是 SQLite-backed Eidos File。
...
当 `.md` 和 `.eidos` 稳定后，将 `.eidos/db.sqlite3` 收缩为私有状态库。
...
只适用于当前隐藏数据库模型。在 Space/Eidos File mode 中，它应该被替换为 Space-level 用户文件追踪，并显式追踪 `.eidos/extensions/**` 这类稳定 Eidos 项目文件。

## 关键决策

1. Eidos File 是用户可见文件，不是隐藏 `.eidos` 数据库。
...
6. `.eidos` 文件是带 Eidos 元数据的 SQLite 数据库。
7. 扩展源码属于 `.eidos/extensions/**`，扩展运行时状态属于 `.eidos/cache/**` 和 `.eidos/state/**`。
8. Eidos Changes UI 先展示 path-level changes，再在展开 Eidos File 时展示内部变更。

## 开放问题

1. 默认 Space tracking 应该是宽规则 `**/*` 搭配 ignore，还是显式规则 `**/*.md`、`**/*.eidos`、`assets/**`？
2. 哪些 `.obsidian/` 文件应该默认进入版本管理？
3. Eidos File attachments 应该作为普通 sibling files、托管 assets 目录，还是 Eidos File 专属 payload 目录？
4. 一个 Space 应该默认有一个 Eidos File、多个 Eidos File，还是两者都支持？
5. 当前 `eidos__tree` 模型在 file-backed Markdown 中还保留多少？
6. 默认扩展源码目录是否就叫 `.eidos/extensions/`，还是应该可配置？
7. 对依赖 `eidos__docs` 的现有 Eidos spaces，精确迁移路径是什么？

## 已证明的 Vertical Slice 与推荐下一步

下面的 vertical slice 已经实现，并由可重复的真实 Eidos File/Graft smoke test 覆盖：
...
- Eidos 可以打开 Space。
- Eidos 可以编辑 `note.md`。
- Eidos 可以打开并编辑 `tasks.eidos`。
- Graft status 显示 `note.md`、`tasks.eidos` 和 `assets/image.png`。
- 展开 `tasks.eidos` 可以看到 table-level 和 row-level changes。
- `.eidos/sessions/**` 永远不会出现在 status 中。

紧凑的 Eidos File 编辑面已经替换为现有 production Grid 交互层的 adapter。多 table/field
...
验收链路已经完成。本 RFC 中 Eidos File v1 已无剩余交付 gate；文件化扩展保持独立并暂缓。
