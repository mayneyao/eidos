...
| Graft versioning      | Local workflow accepted | Changes, staging, commit, diff, paged history, file/folder discard, path/Space restore, and Base inspection pass native Desktop acceptance. Remote sync and conflicts remain.                |
...
The next order is remote sync/conflicts, followed by file-based extensions and
the remaining Base/product refinements.
