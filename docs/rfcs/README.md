...
| Product UX            | Delivery closure      | Files/Version, Diff/History, Settings, and migration flows are accepted. Base now has atomic creation plus persisted Grid/Gallery/Kanban views, but full table-view parity and native lifecycle acceptance remain.                 |
| Base format/runtime   | Delivery closure      | The standalone package supports structured queries, persisted multi-layout views, rich file/relation/live-formula/lookup fields, validated previews, table/field lifecycle, batch deletion, and streamed imports.                  |
...
The next order is Base delivery closure: complete the creation/edit/restart/
version/restore acceptance path and recover the high-value interactions from
the existing Eidos table. File-based extensions follow that gate.
