...
Grid, Gallery, Kanban, and Calendar MUST remain interactive without materializing all
...
| ------------------------------------ | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| Text, URL                            | scalar text; eligible URL is a policy-checked link     | editable stored value                                                                                  |
| URL with `display.kind="image"`      | Host-leased image, then inert URL fallback             | editable URL; presentation never rewrites the value                                                    |
...
| File                                 | image preview, then type icon, then inert URI fallback | editable only through Host acquisition (`acquireAsset` or `acquireRemoteAsset`) and a Runtime mutation |
...
eligible value with a link affordance, including link color and an underline
when its text is hovered, and a primary pointer click on the rendered text
MUST dispatch directly to the embedding Host's URL activator without first
opening the cell editor. A click outside the rendered text retains ordinary
Grid selection behavior; editing remains available through the Grid's
explicit edit gesture. The same rule applies to a scalar URL Formula/Lookup
result presented as URL, except that its value remains read-only.

The UI passes the exact raw value and MUST NOT fetch, probe, normalize, or
write it during activation. The embedding Host MUST independently validate
the destination and MAY apply a stricter navigation policy. Relative values,
unsupported schemes, credential-bearing values, malformed values, values over
the negotiated UI bound, or a missing Host activator remain inert and
copyable. Activation failure is reported without changing the value. A
desktop Host opens an accepted destination through its privileged external
navigation boundary; it does not rely on renderer `window.open`.

An ordinary URL Field remains inert with respect to network fetching and MUST
NOT be fetched merely because a cell becomes visible. A scalar URL Field or
scalar URL Formula/Lookup result whose Field settings declare
`display.kind="image"` MAY request
`resolveUrlImage` only for a non-empty absolute `https:` value, while its cell
is inside the bounded rendered window, and only when the Host advertises
`https`, exposes that optional operation, and current policy allows it. Grid
and image-capable card surfaces use `AssetPresenter.loadImage`, deduplicate
equal URLs, bound concurrency, cancel stale generations, and release the lease
after decode or eviction.
After releasing the lease, the UI SHOULD retain recently decoded images in a
session-scoped, exact-URL-keyed, bounded memory cache so a row leaving and
re-entering the rendered window does not cause another network request. That
cache MUST have both entry-count and estimated decoded-byte bounds, use bounded
eviction such as LRU, and clear when its Host session or authorization context
ends; it MUST NOT become canonical state, a cross-session or
cross-authorization cache, or persistent storage.
The URL remains the editable/copyable/exported value. Empty, non-HTTPS,
denied, unsupported, failed, or unavailable values fall back to inert text and
never mutate canonical state. Record Labels and Relation labels always use the
text fallback and never trigger this image flow.

CSV import MAY suggest this setting from bounded header names and URL syntax,
but MUST NOT fetch or probe any image during planning or import. The setting is
Field-wide, so Grid and other image-capable surfaces share the same default
presentation; View layout continues to own dimensions, visibility, card cover,
and fitting. Host authorization is local capability state and is not implied by
the persisted setting.

File presentation follows this deterministic ladder:
...
When optional `acquireRemoteAsset` is present and `https` is listed in
`assetWriteSchemes`, an editable File surface SHOULD expose an explicit
"add from URL" action. The action accepts one absolute HTTPS URI and an
optional display filename, then submits both to the Host. Plain paste, CSV
import, rendering, scrolling, and opening a File editor MUST NOT implicitly
invoke this operation. The Host returns a Runtime-allocated `FileEntry`; the
UI applies the same unchanged-object and failed-mutation rules as
`acquireAsset`. Absence of the operation or capability hides or disables the
action without making existing HTTPS File entries invalid.
