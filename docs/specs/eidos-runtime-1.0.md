...
The URL `display.kind="image"` setting is presentation metadata only. Runtime
preserves and returns it, but CSV planning, validation, query, mutation,
Formula/Lookup evaluation, search, and export perform no image probe or
network request. CSV image-column inference, when provided, uses only bounded
header/value syntax and stores the original URL strings unchanged.
